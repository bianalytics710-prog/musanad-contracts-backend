...
> **Generated:** 2026-05-05.
> **Predecessor:** M5 (`schema_migrations.version=53`).
> **Schema migration window:** 054 → 056 (3 designed) + 057 (M6-DB-IMPL-DEFECT-1 patch). Final `schema_migrations.version=57` on both `test` and `m0-foundation` branches.
> **Codex review:** SKIPPED (Dexian decision 2026-05-04; **6th consecutive validation** through M2/M3/M4/M5/M6). Stage 2 + Stage 4 absorb the safety net via S2-16..S2-23 (with **S2-22b column-existence-via-JOIN-target tracing** recommended for codification post M6-DB-IMPL-DEFECT-1).

This dictionary documents the database surface introduced by M6: **0 new tables, 0 ALTER TABLE, 0 new columns**. The module is read-only — it composes existing M0..M5 telemetry through 4 plain views and 10 new fn_'s (all SECURITY INVOKER) into role-shaped dashboard snapshots. 1 new permission code + 3 role grants. 1 new RLS SELECT policy on the borrowed `schema_migrations` table (ARCH-NEW-3 option (c)). No new cron driver (M6 is event-driven; aggregates compute on demand at request time).

For the wire surface see [`api/openapi.yaml`](api/openapi.yaml) (M6 Dashboards + M6 Admin Health tags). For the implementation handoff see [`dev-handoff.md`](dev-handoff.md) (M6 Implementation Notes section). For the Lovable transformation log see [`lovable-handoff.md`](lovable-handoff.md) (M6 Migration section).

---

## Tables

**None.** M6 is a read-only module that does not introduce any new persistent storage. The DB-IMPL-I-1 trap (TEXT/VARCHAR PK breaking `fn_audit_trigger.NEW.id`) is N/A in M6 — there are no new tables to attach audit triggers to. The `id BIGSERIAL` invariant is preserved across the platform (last reaffirmed in M5).

---

## Views (4 new — Q3 = plain VIEWs)

All 4 views are plain (non-materialised) — query-time projections suitable for dashboards where freshness > pre-compute. No `CREATE INDEX` on views; the planner uses indexes on the underlying tables.

### `vw_contract_status_summary`
**Underlying tables:** `contract`.
**Purpose:** Summary aggregate by `contract.status` — used by `fn_dashboard_admin.kpis.totalContractsByStatus` and `fn_dashboard_executive.kpis.contractsByStatus`. Filters `is_active = TRUE`.
**Columns:** `status`, `cnt`, `sum_value_aed`, `min_created_at`, `max_updated_at`.

### `vw_approval_queue_metrics`
**Underlying tables:** `approval_step`.
**Purpose:** PERCENTILE-based queue-velocity metrics — median + p95 wait hours for `pending` steps. Used by future module fact-tables and the legal-counsel / admin dashboards as needed.
**Columns:** `total_pending_steps`, `median_time_pending_hours`, `p95_time_pending_hours`.
**S2-22-FIX-2b:** PERCENTILE_CONT computes against `step.created_at` (NOT non-existent `assigned_at`).

### `vw_signature_status_summary`
**Underlying tables:** `signature_invitation`, `signature_event`.
**Purpose:** Signature time-to-sign metrics derived from invitation→event traversal. Median + p95 hours from `invitation_sent_at` to `event.created_at`.
**Columns:** `total_invitations`, `median_time_to_sign_hours`, `p95_time_to_sign_hours`.
**S2-22-FIX-1c + FIX-5:** LEFT JOIN narrowed to `se.event_type='signed' AND se.is_active=TRUE`; uses `invitation_sent_at` (not `sent_at`) and `created_at` (not `signed_at`).

### `vw_regulatory_impact_summary`
**Underlying tables:** `regulatory_impact`.
**Purpose:** Open-impact aggregates by severity (joined to `regulatory_update.severity`). Used by `fn_dashboard_legal_counsel.kpis.criticalSeverityCount` and `fn_dashboard_admin`.
**Columns:** `severity`, `open_count`, `total_count`.
**CRIT-1:** Filters `WHERE resolved = FALSE` (column is BOOLEAN; there is no `resolved_at` column).

### View NOT created — `vw_ai_cost_rollup`
**Why dropped (Q5 lock):** Single-source-of-compute decision. `fn_ai_request_log_cost_report` (M4 043) already returns the same shape. M6 wraps that fn_ via `fn_dashboard_ai_cost_summary` (S11) AND inlines a slice into `fn_dashboard_executive.kpis.aiCostUsdWindow` (S7). No view; no duplicate compute path. Documented in `db-implementation-summary.json` `viewsDroppedRationale`.

---

## Functions (10 new — all SECURITY INVOKER)

All 10 fn_'s: `REVOKE ALL ON FUNCTION ... FROM PUBLIC; GRANT EXECUTE ... TO neondb_owner;` plus `SET search_path = public, pg_temp`. **Zero new PUBLIC EXECUTE grants** (S2-21 mandatory; PUBLIC allowlist stays at the M3 set of 5 names — verified live; **6th consecutive validation** since codification at M4).

### Read functions (10)

| Function | Returns | Purpose |
|---|---|---|
| `fn_dashboard_admin(p_window_days INTEGER DEFAULT 30) → JSONB` | Snapshot | S1 + S13 — System-wide KPIs + trends. Role gate: platform_admin OR Super Admin. windowDays range 1..365 (22023 → AC-S1-06). `trends.approvalDecisionsByDay` uses present-tense `decision='approve'/'reject'` (S2-22-WARN-1-FIX). `trends.contractsCreatedByDay` gap-filled via `generate_series`. |
| `fn_dashboard_drafter(p_window_days INTEGER DEFAULT 30) → JSONB` | Snapshot | S2 — Drafter dashboard scoped to `drafted_by = current_user_id`. 4 KPIs + 2 list slots. Role gate: contract_drafter OR platform_admin OR Super Admin. |
| `fn_dashboard_approver(p_window_days INTEGER DEFAULT 30) → JSONB` | Snapshot | S3 — Approver dashboard. **post-057 patch — JOIN chain corrected:** `approval_step JOIN approval_chain ON ch.id = step.approval_chain_id JOIN contract ON c.id = ch.contract_id` (M6-DB-IMPL-DEFECT-1). Effective-assignee derived as `COALESCE(delegated_to, reassigned_to, approver_user_id)` (S2-22-FIX-2a override-chain). Velocity AVG via `EXTRACT(EPOCH FROM (ad.decided_at - step.created_at))/3600` (S2-22-FIX-2b — was non-existent `assigned_at`). |
| `fn_dashboard_legal_counsel(p_window_days INTEGER DEFAULT 30) → JSONB` | Snapshot | S4 — Legal counsel dashboard. `auditSummary` keys are live `audit_log.table_name` values (S2-22-FIX-4). Audit slot gates on `fn_current_user_has_permission('audit.read')` per CRIT-4 — NULL when permission absent (NOT `audit.read.all`; M4 drift carry-forward as ARCH-NEW-1). `openRegulatoryImpacts` filters `WHERE resolved = FALSE` per CRIT-1. `templateUsageThisWindow` returns `{value:0, placeholder:true}` per AC-S4-05 (templates module future). |
| `fn_dashboard_recipient(p_window_days INTEGER DEFAULT 30) → JSONB` | Snapshot | S5 — Recipient dashboard. Identifies recipient via `lower(signature_party.signer_email) = lower(current_user.email)`. `signedByMeWindow` uses `signature_event.actor_user_id + created_at + event_type='signed' AND is_active=TRUE` (S2-22-FIX-1; was `signer_user_id`/`signed_at`/`outcome` — none exist). `pendingSignatures5` aliases `invitation_sent_at AS sentAt`, `invitation_expires_at AS expiresAt` (S2-22-FIX-5a). `myObligationsCount` returns `{value:0, placeholder:true}`. DN-19: external-only signers excluded from `signedByMeWindow` (their `actor_user_id IS NULL`). |
| `fn_dashboard_router() → JSONB` | Routing payload | S6 — `{ userId, primaryRole, dashboardKey, permissionsSummary }`. Role extraction via `COALESCE(v_user->'role'->>'name', v_user->>'roleName', 'unknown')` per S2-22-WARN-3-FIX — live `fn_user_get_by_id` returns nested `role:{id,name}`, NOT flat `roleName`. Decision tree codified at db-design.md §3.6 step 4. Default fallback: `'recipient'`. |
| `fn_dashboard_executive(p_window_days INTEGER DEFAULT 90) → JSONB` | Snapshot | S7 — Executive enterprise overview. Role gate: `executive` OR `platform_admin` OR `Super Admin` OR `fn_current_user_has_permission('insights.executive')`. `kpis.aiCostUsdWindow` is **inline** (Q5 lock — wraps `fn_ai_request_log_cost_report`); returns NULL with no error when caller lacks `ai.observability.read` (AC-S7-05 panel marker). 90-day cap inherited from M4 via `LEAST(p_window_days, 90)`. CRIT-3: `contract_select_role_aware` includes `executive` — fn stays SECURITY INVOKER, no DEFINER carve-out. |
| `fn_dashboard_executive_anomalies_history(p_limit INTEGER DEFAULT 10) → JSONB` | Cache view | S8 — Reads M4 `ai_insight` rows where `entity_type='executive_anomalies'` via `fn_ai_insight_list(1, p_limit, 'executive_anomalies', NULL, NULL, NULL, FALSE)`. Empty array (NOT 404) when cache empty (AC-S8-02). Detection refresh action OWNED BY M4 — DO NOT add a write endpoint here. |
| `fn_dashboard_ai_cost_summary(p_window_days INTEGER DEFAULT 30) → JSONB` | Cost rollup | S11 — Standalone AI cost endpoint per Agent 5 DASH-OI-G. Wraps M4 `fn_ai_request_log_cost_report(CURRENT_DATE - p_window_days, CURRENT_DATE, FALSE)`. windowDays range **1..90** matching M4 cap (AC-S11-05). `cacheHitRatioOverall` NULL when `totalRequestsWindow=0` (S2-18). Pre-gate at route layer + defence-in-depth fn body re-gate on `ai.observability.read`. |
| `fn_health_check() → JSONB` | Health snapshot | S12 — Admin observability probe. Returns `{db, ai, overall}`. **Stays SECURITY INVOKER** per ARCH-NEW-3 option (c) — the new `schema_migrations_select_admin` permissive SELECT policy (migration 054) is what enables `db.latestMigration = MAX(version)` for admin callers. Audit probe DROPPED in design Patch Round 1 per S2-22-WARN-2-FIX (audit_log.action enum is INSERT/UPDATE/DELETE only; literal 'ERROR' could never match). Error signal sourced exclusively from `ai_request_log.outcome IN ('error','timeout','rate_limited','cancelled')`. |

### Cross-module fn-to-fn calls

M6 fn_'s only invoke existing M0..M5 fn_'s (no signature changes — S2-19 N/A):

| M6 caller | Callee | Module | Notes |
|---|---|---|---|
| `fn_dashboard_executive` | `fn_ai_request_log_cost_report` | M4 (043) | inline aiCostUsdWindow; LEAST clause caps window. |
| `fn_dashboard_executive` | `fn_current_user_has_permission` | M0 | gate on `insights.executive` OR `ai.observability.read`. |
| `fn_dashboard_ai_cost_summary` | `fn_ai_request_log_cost_report` | M4 (043) | full pass-through. |
| `fn_dashboard_ai_cost_summary` | `fn_current_user_has_permission` | M0 | gate on `ai.observability.read`. |
| `fn_dashboard_executive_anomalies_history` | `fn_ai_insight_list` | M4 (043) | passes `(1, p_limit, 'executive_anomalies', NULL, NULL, NULL, FALSE)`. |
| `fn_dashboard_router` | `fn_user_get_by_id` | M0 | reads nested `role:{id,name}` shape per S2-22-WARN-3-FIX. |
| All 10 M6 fn_'s | `fn_current_user_has_permission` | M0 | role + permission gates. |

---

## RLS Policies (1 new)

### `schema_migrations_select_admin` — ARCH-NEW-3 option (c)

| Field | Value |
|---|---|
| Table | `schema_migrations` (M0-owned, borrowed for ARCH-NEW-3 read access) |
| Migration | `054_m6_insights_permissions_and_grants.sql` |
| Command | `SELECT` |
| Type | `PERMISSIVE` |
| Roles | `PUBLIC` (any role; the USING clause narrows by user role) |
| USING clause | Caller's role IN (`platform_admin`, `Super Admin`) AND `current_setting('app.current_user_id', TRUE)` IS NOT NULL |
| Pre-existing | `schema_migrations_deny_all` (RESTRICTIVE) is preserved and continues to block INSERT/UPDATE/DELETE for non-superusers. |

**Why this exists:** `fn_health_check.db.latestMigration` reads `MAX(version) FROM schema_migrations`. Without a permissive SELECT policy, the deny-all RLS would force the read to NULL on a non-superuser pool connection. The architect-recommended option (c) keeps `fn_health_check` as SECURITY INVOKER (no DEFINER carve-out) by adding a narrow permissive SELECT policy alongside the existing deny-all. The deny-all continues to block INSERT/UPDATE/DELETE.

**Verification:** Both branches (`test`, `m0-foundation`) post-apply have BOTH policies. `fn_health_check` called as Super Admin (user 1) successfully reads `MAX(version)=57`. Non-admin callers correctly see `latestMigration=NULL` (or 42501 if their role is also blocked at the fn body).

---

## Permission codes added (1) + role grants (3)

| Code | Module | Action | Grants |
|---|---|---|---|
| `insights.executive` | insights | executive (alternate gate path for `fn_dashboard_executive`) | Super Admin (pre-emptive), platform_admin, executive |

Grants seeded in migration 054 with `ON CONFLICT DO NOTHING` (idempotent re-apply verified).

**Cross-module permission codes consumed by M6 (NOT seeded by M6 — referenced only):**

| Code | Origin | Consumed by |
|---|---|---|
| `audit.read` | M0 | `fn_dashboard_legal_counsel.auditSummary` slot (CRIT-4 lock — NULL when missing). |
| `ai.observability.read` | M4 (044) | `fn_dashboard_executive.kpis.aiCostUsdWindow` (NULL when missing — AC-S7-05 panel marker) AND `fn_dashboard_ai_cost_summary` entrypoint (defence-in-depth pre-gate at route + fn body). |

> **`contract_recipient` is not granted any M6 permission.** Per the dashboard router decision tree (§3.6), `contract_recipient` lands on the `recipient` dashboard via the default fallback branch and consumes `fn_dashboard_recipient` only. No M6 endpoint requires a permission `contract_recipient` lacks.

---

## Migrations applied (4 total — 054..057)

| # | File | Contents |
|---|---|---|
| 054 | `054_m6_insights_permissions_and_grants.sql` | **MANDATORY FIRST.** INSERT 1 new permission code (`insights.executive`) + 3 role_permission grants (Super Admin pre-emptive + platform_admin + executive). Plus the `schema_migrations_select_admin` permissive SELECT RLS policy (ARCH-NEW-3 option c). All `ON CONFLICT DO NOTHING`. Mirrors M2 028 / M3 037 / M4 044 / M5 046 atomic pattern (permission seed always migration #1 of the module window). |
| 055 | `055_m6_dashboard_views.sql` | CREATE the 4 plain VIEWs (`vw_contract_status_summary`, `vw_approval_queue_metrics`, `vw_signature_status_summary`, `vw_regulatory_impact_summary`). All views are non-materialised (Q3 lock). Idempotent via `CREATE OR REPLACE VIEW`. |
| 056 | `056_m6_dashboard_functions.sql` | All 10 `CREATE OR REPLACE FUNCTION` statements with per-fn EXECUTE GRANT/REVOKE. Mandatory dedicated `NNN_fn_<entity>_functions.sql` per Agent 4 v2.1 quality check #14. **Zero new PUBLIC EXECUTE grants** (S2-21 — count stays at 5). |
| 057 | `057_m6_fix_fn_dashboard_approver_contract_join.sql` | **M6-DB-IMPL-DEFECT-1 patch.** `CREATE OR REPLACE FUNCTION fn_dashboard_approver(INTEGER)` correcting the JOIN-target column drift on `step.contract_id`. Live `approval_step` has only `approval_chain_id`; the contract reference travels via `approval_chain.contract_id`. New JOIN chain: `approval_step step JOIN approval_chain ch ON ch.id = step.approval_chain_id JOIN contract c ON c.id = ch.contract_id`. Signature unchanged; output JSONB shape unchanged. Mirrors M3 038/039 + M4 045 + M5 053 named-fix-migration precedent. Codifies S2-22b extension: **join-target-column tracing** — every JOIN target.column reference must be verified against the live DDL of the target side, not just the source side. |

All 4 migrations applied to `test` (`br-billowing-boat-ajq9m0g6`) AND `m0-foundation` (`br-snowy-brook-aje2ehtl`). Final `schema_migrations.version=57` on both branches.

---

## Cross-module writes (CMW)

**None.** M6 is read-only by design (Q11 = DEFER). The 4 sibling-fn FK pre-check parity gaps surfaced as M5 forward-looking carry-forward remain DEFERRED to a dedicated M5-patch slot — NOT bundled into M6 (per Q11 architect default; bundling write-fn patches into a read-only module dilutes module attribution and complicates State Writer scope).

| ID | Target | Operation | Migration |
|---|---|---|---|
| n/a | (no M0..M5 tables modified by M6) | (no DDL in M6 against borrowed tables) | (none) |

The closest thing to a CMW is the new `schema_migrations_select_admin` RLS policy added to the M0-owned `schema_migrations` table — additive permissive policy alongside the existing deny-all. Documented under "RLS Policies" above.

---

## Sensitive-field redaction (M6 additions)

DB-layer (`fn_audit_trigger v_redact_fields`): **NOT EXTENDED.** M6 is read-only and writes no audit_log rows directly. The fn_ outputs flow through controllers as JSONB pass-through; no new column path is introduced.

App-layer (Pino redact in `src/utils/logger.util.ts` SENSITIVE_PATHS): **NOT EXTENDED.** M6 dashboards return aggregates only. List projections include `id` / `contractNumber` / `titleEn` / `titleAr` / `status` / `valueAed` / `updatedAt` — none of which appear in `project.config.json` `sensitiveFields[15]`. The standing M0..M5 redact contract is sufficient.

**Intentional non-redacts (carry-forward from M2):** `lastDecisionNote` in `DrafterAwaitingActionRow` is the most recent reject / request_resubmission note from M2's approval_decision; M2 already pino-redacts this on logs across 14 paths — the API response includes it for the drafter's consumption (audit-redacted by `fn_audit_trigger` during the original M2 write).

---

## S2-21 invariant — PUBLIC EXECUTE allowlist (live verified)

PUBLIC EXECUTE on fn_'s remains at exactly the M3 set of 5 names — **6th consecutive validation** (M2/M3/M4/M5/M6). Verified by enumerating `pg_proc.proacl` matching `'=X/owner'` pattern across both `test` and `m0-foundation` branches post-migration 057.
...
Method note: `pg_proc.proacl` enumeration is preferred over `has_function_privilege(0, ...)` (rejected because OID 0 returns false positives for fn_'s with NULL/owner-only ACLs).

---

## S2-22b codification recommendation — JOIN-target-column tracing

The M6 DEFECT-1 escape root cause was a JOIN-target column reference that LOOKED valid (the source-side column was correctly named) but the TARGET-side column did not exist on the named table. Specifically: `approval_step.contract_id` was referenced; live `approval_step` has only `approval_chain_id` — the contract reference travels via `approval_chain.contract_id`. The current S2-22 sweep verifies columns on the named tables but missed this implicit cross-table FK semantic.

**Proposed codification (S2-22b):** for every JOIN clause `<table_a> JOIN <table_b> ON <cond>`, every column referenced anywhere in the fn body that is QUALIFIED with the target-side alias (e.g., `step.contract_id` after `JOIN approval_chain ch ON …`) MUST be verified against the live DDL of the target side, not just the source side. Where ambiguity exists ("is this column on table_a or table_b?"), trace through the FK graph in `project-artifacts/database/tables/*.sql`.

Recommendation logged for memory promotion at module close.

---

## DB-IMPL-I-1 not recurring in M6

N/A — M6 introduces ZERO new tables. The DB-IMPL-I-1 trap (TEXT/VARCHAR PK breaking `fn_audit_trigger.NEW.id`) is structurally inapplicable. The platform-wide BIGSERIAL PK invariant is preserved.

---

## NULL-safe equality (S2-18) — sustained

`fn_dashboard_approver.kpis.pendingMyApprovalCount` uses `COALESCE(delegated_to, reassigned_to, approver_user_id) IS NOT DISTINCT FROM v_user_id` per S2-22-FIX-2a + S2-18 (admin-forced override / voluntary delegation chain). `fn_dashboard_ai_cost_summary.cacheHitRatioOverall` returns NULL when `totalRequestsWindow=0` per S2-18 NULL semantic (matches M4 `fn_ai_request_log_cost_report` parent semantic). Both verified live in DB-Impl S2-22 column-existence probes.

---

## System-actor sentinel (S2-20) — N/A in M6

M6 introduces NO new cron driver. The 3 existing crons (M2 approval-escalation, M3 signature-expiration, M4 ai-insight-eviction) remain wired in `server.ts` unchanged. M6 dashboards compute on demand at request time — no scheduled refresh path.

---

## Concurrency primitive verification (S2-17) — N/A in M6

M6 is read-only. No `SELECT FOR UPDATE`, no `pg_advisory_*` locks, no atomic gate+commit patterns introduced. The existing M2 + M3 + M4 + M5 concurrency primitives are unchanged.

---

## Key DB Impl outcomes (M6)

| ID | Severity | Topic | Outcome |
|---|---|---|---|
| **M6-DB-IMPL-DEFECT-1** | CRITICAL — fixed in 057 | JOIN-target column drift on `step.contract_id` | Caught at first runtime probe (`SELECT fn_dashboard_approver(30)` → SQLSTATE 42703 `column step.contract_id does not exist`). Patched in migration 057 (`CREATE OR REPLACE FUNCTION fn_dashboard_approver(INTEGER)` with corrected `approval_step JOIN approval_chain JOIN contract` chain). Output JSONB shape unchanged. Re-probed with user 1262 (1 pending step) — returns full contract context (contractId=4076, contractNumber='PROBE-031-E-1777885059', valueAed=100000, hoursWaiting=53.1). Codifies S2-22b: JOIN-target-column tracing. |
| **M6-DB-PATCH-ROUND-1** | n/a (caught in design Stage 2) | 6 column-existence FAILs + 3 WARNs caught at design time | Stage 2 (round 1) caught 6 critical column references that don't exist on the live schema (`signature_event.signer_user_id` / `signed_at` / `outcome`; `approval_step.assigned_to` / `assigned_at`; `approval_decision.decider_user_id`; `audit_log.entity_type`; `signature_invitation.sent_at` / `expires_at`) plus 3 WARN clarifications (`audit_log.action='ERROR'` literal dead-on-arrival; `decision IN ('approved','rejected')` past-tense literals; `v_user->>'roleName'` flat-key extraction against nested live shape). All 9 resolved at design time via in-place surgical edits to `db-design.md` (Patch Round 1; 2026-05-05). Stage 2 round 2 PASS. Validates the S2-22 design-time discipline. |
| **M6-DBI-INFO-1** | INFO | S2-21 sustained at 5 | 6th-consecutive-clean module. PUBLIC EXECUTE grants verified live on both branches via `pg_proc.proacl` enumeration. |
| **M6-DBI-INFO-2** | INFO | ARCH-NEW-3 option (c) live verified | `schema_migrations_select_admin` policy applied on both branches; `fn_health_check` successfully reads `MAX(version)=57` when called as Super Admin; deny-all RLS continues to block INSERT/UPDATE/DELETE for non-superusers. fn stays SECURITY INVOKER (no DEFINER carve-out). |
| **M6-DBI-INFO-3** | INFO | seed idempotency verified | Migration 054 re-applied to m0-foundation after test branch — `ON CONFLICT DO NOTHING` left rows unchanged. Standard pattern (M2/M3/M4/M5 precedent). |
...
# M7 — OSINT Source Framework + Adapter Protocol — Data Dictionary

> **Module:** M7 (CR-A; CRIP Wave 1 chassis)
> **Generated:** 2026-05-09
> **Source migrations:** `database/migrations/100_m7_permissions_seed.sql` through `database/migrations/107_m7_osint_functions.sql` (8 files; 0 impl-time patches)
> **Schema version:** `schema_migrations.version = 107` on both `m0-foundation` + `test` branches

---

## Tables (5 new + 1 renamed-and-extended)

### 1. `tenant`

**Purpose.** Minimal multi-tenancy bootstrap (Q2 lock). Single ADNOC row in CR-A; CR-C extends with branding, hierarchy, hash-chained audit.
**Owned by:** M7. **Audit columns:** `created_at`, `updated_at`, `created_by`, `updated_by`. **Audit trigger:** **OFF (CC1 / OQ-8 carve-out)** — `tenant.id` is UUID and `audit_log.record_id` is BIGINT; direct INSERT would fail. Tenant audit deferred to CR-C hash-chained `audit_log` replacement (UUID-compatible record_id). **RLS:** FORCE; 2 policies (self-row read via `app.current_tenant_id` GUC + platform_admin sees all).

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | UUID | PRIMARY KEY DEFAULT gen_random_uuid() | UUID PK. ADNOC seed = `00000000-0000-0000-0000-000000000001`. |
| `slug` | VARCHAR(50) | NOT NULL UNIQUE | Stable string handle. ADNOC slug = `adnoc`. |
| `display_name` | VARCHAR(200) | NOT NULL | Display name. ADNOC = `ADNOC`. |
| `config_pack` | VARCHAR(50) | NOT NULL DEFAULT 'default' | Drives seed selection. ADNOC tenant uses `config_pack='adnoc'`. |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | Soft delete flag. |
| audit cols | (see above) | | |

**Indexes.** PK + UNIQUE(slug) + `idx_tenant_active ON tenant(id) WHERE is_active = TRUE`.

**Seed (migration 101).** 1 row: ADNOC.

---

### 2. `osint_source`

**Purpose.** Source registry — admin-managed catalogue of external feeds. One row per logical source (ofac_sdn, eu_consolidated, rss_<slug>, commodity_crude, gdelt_v2, fx_usd_aed, etc.). Tenant-scoped via `app.current_tenant_id` GUC.
**Owned by:** M7. **Audit trigger:** ON (`audit_osint_source_changes`). **RLS:** FORCE; 4 policies (SELECT via `source.read`; ALL via `source.manage`; deny-direct-delete RESTRICTIVE; tenant-scope predicate on every policy).

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGSERIAL | PRIMARY KEY | |
| `tenant_id` | UUID | NOT NULL REFERENCES tenant(id) ON DELETE RESTRICT | |
| `source_id` | TEXT | NOT NULL | Stable per-tenant string handle (e.g. `ofac_sdn`). Immutable post-create — `fn_osint_source_update` rejects payloads containing sourceId with 22023. |
| `display_name` | VARCHAR(200) | NOT NULL | |
| `display_name_ar` | VARCHAR(200) | | Arabic display name for i18n parity. |
| `kind` | TEXT | NOT NULL CHECK ∈ (sanctions/news/weather/commodity/fx/social/regulatory/internal) | UI dropdown source. |
| `url` | TEXT | | **[SENSITIVE]** May embed query-string secrets. Pino-redacted. |
| `format` | TEXT | NOT NULL CHECK ∈ (xml/csv/json/rss/api) | UI dropdown source. |
| `refresh_seconds` | INTEGER | NOT NULL CHECK ≥ 60 | Per-row override of adapter-class default. **Per-row always wins** (Q-NEW4 lock). |
| `source_reliability` | NUMERIC(3,2) | NOT NULL CHECK ∈ [0,1] | |
| `enabled` | BOOLEAN | NOT NULL DEFAULT TRUE | |
| `rate_limit` | JSONB | | Annex B.5.1 shape: `{callsPerMinute, burst, minIntervalMs, respectRetryAfter}`. |
| `severity_mapping` | JSONB | | Source-specific rules; first-match-wins evaluation. |
| `geography_filter` | JSONB | | GDELT relevance filter: `{countryIn[], themeIn[], actorIn[]}`. |
| `licensing_note` | TEXT | | |
| `metadata` | JSONB | NOT NULL DEFAULT '{}' | Adapter-specific extra config (sub-feed list, marker list, currency-pair list). |
| `data_classification` | VARCHAR(20) | NOT NULL DEFAULT 'demo' CHECK ∈ (demo/pilot/production) | Annex B.2.1 + §15 invariant 5 alignment. |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | Soft delete flag. |
| audit cols | (see above) | | |

**Indexes.** PK + UNIQUE(tenant_id, source_id) + `idx_osint_source_tenant_id` + `idx_osint_source_active WHERE is_active = TRUE` + `idx_osint_source_tenant_kind(tenant_id, kind)` + `idx_osint_source_tenant_enabled(tenant_id, enabled, is_active)` + `idx_osint_source_created_by` + `idx_osint_source_updated_by`.

**Seed (migration 106).** 13 rows for ADNOC tenant: 4 sanctions (ofac_sdn, eu_consolidated, un_security_council, uk_hmt) + 6 RSS sub-feeds (rss_reuters_energy, rss_lloyds_force_majeure, etc.) + commodity_crude + gdelt_v2 + fx_usd_aed.

---

### 3. `source_credential`

**Purpose.** Per-source credential indirection. Stores `credential_kind` + `credential_ref` (env var name or vault path); never plain-text secret. Strict RLS — only `source.manage` callers can modify.
**Owned by:** M7. **Audit trigger:** ON (`audit_source_credential_changes`). `credential_ref` redacted to `[REDACTED]` in audit_log payloads via `fn_audit_trigger.v_redact_fields` (migration 102). **RLS:** FORCE; 2 policies (manage-only + deny-direct-delete RESTRICTIVE).
...
| `credential_kind` | TEXT | NOT NULL CHECK ∈ (api_key/oauth_token/basic_auth/none) | |
| `credential_ref` | TEXT | | **[SENSITIVE]** KMS-style indirection. Pattern: `env:VARNAME` OR `vault:path`. Plain-text values rejected by fn with 22023. Listed in project.config.json sensitiveFields (AE3) AND fn_audit_trigger redact list (AE1). |
| `last_rotated_at` | TIMESTAMPTZ | | |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |
| audit cols | (see above) | | |

**Indexes.** PK + UNIQUE(tenant_id, osint_source_id) + `idx_source_credential_tenant_id` + `idx_source_credential_osint_source_id` + `idx_source_credential_active WHERE is_active = TRUE`.

**Seed (migration 106).** 2 rows: env:COMMODITY_API_KEY (commodity_crude) + env:FX_API_KEY (fx_usd_aed).

---

### 4. `osint_signal` (RENAMED from `impact_signal` + EXTENDED — STRATEGY-A preserve-superset)

**Purpose.** Annex B.2.1 normalised signal store. The original 13 R-LC business columns are physically retained as nullable additions; the 19 new Annex B.2.1 columns added on top. R-LC fn_'s read this through the `impact_signal` view shim (Section 4) without rewrite.
**Owned by:** M7 (transferred from R-LC; preserve-superset). **Audit trigger:** ON (`audit_osint_signal_changes` — renamed from `audit_impact_signal_changes`). `raw_payload` redacted in audit_log per AE1. **RLS:** FORCE; 4 policies (tenant-isolation SELECT via `signal.read.all` OR `contract.read.department` OR `contract.edit` for R-LC back-compat; deny-direct-INSERT RESTRICTIVE — writes only via DEFINER `fn_osint_signal_upsert`; deny-direct-update RESTRICTIVE; deny-direct-delete RESTRICTIVE).

**New columns (added in migration 104):**

| Column | Type | Constraints | Description |
|---|---|---|---|
| `tenant_id` | UUID | NOT NULL after backfill REFERENCES tenant(id) | |
| `osint_source_id` | BIGINT | NULLable REFERENCES osint_source(id) ON DELETE RESTRICT | NULL only for the 17 R-LC manual_curated back-fill rows (Q-DA3 — they predate osint_source seeding). Partial FK index `WHERE osint_source_id IS NOT NULL` (Stage 2 S2-7 patch). |
| `source_id` | TEXT | NOT NULL after backfill | Denormalised; mirrors `osint_source.source_id` for query perf. |
| `source_reliability` | NUMERIC(3,2) | NOT NULL after backfill CHECK ∈ [0,1] | Snapshot at insert time. |
| `fetched_at` | TIMESTAMPTZ | NOT NULL after backfill | Wall-clock when worker fetched the batch. |
| `event_date_v2` | TIMESTAMPTZ | | Annex B.2.1 TZ-aware event date. The original R-LC `effective_date DATE` retained for back-compat. |
| `kind` | TEXT | NOT NULL after backfill CHECK ∈ (geopolitical/sanctions/weather/commodity/fx/logistics/esg/regulatory/news/internal) | |
| `signal_kind_subtype` | TEXT | | Discriminator: `manual_curated` for the 17 R-LC seed rows; NULL or `milestone_slippage` / `sla_breach` for live OSINT. |
| `title` | TEXT | | Normalised single-locale title; null for R-LC rows (use `title_en`). |
| `summary` | TEXT | | |
| `geographies` | JSONB | NOT NULL DEFAULT '[]' | Array of `GeoReference` shapes — `{isoCountry, regionCode?, free?}`. GIN(jsonb_path_ops). |
| `affected_entities` | JSONB | NOT NULL DEFAULT '[]' | Array of `EntityReference` shapes — `{entityType, name, identifier?, partyId?}`. GIN(jsonb_path_ops). |
| `severity_v2` | TEXT | NOT NULL after backfill CHECK ∈ (informational/low/medium/high/critical) | Annex B.2.1 ordered enum. The original R-LC free-text `severity` retained for back-compat. |
| `confidence` | NUMERIC(3,2) | NOT NULL after backfill CHECK ∈ [0,1] | |
| `url` | TEXT | | |
| `raw_payload` | JSONB | NOT NULL after backfill | **[SENSITIVE]** Original source payload. Redacted in logs + audit_log; exposed in API response per design (legal_counsel needs SDN entry text — CR-H consumer). |
| `dedup_hash` | TEXT | NOT NULL after backfill | `sha256(source_id \|\| '\|' \|\| isoformat(event_date OR fetched_at) \|\| '\|' \|\| lower(trim(title)))`. UNIQUE(tenant_id, dedup_hash) drives idempotent upsert. |
| `data_classification` | VARCHAR(20) | NOT NULL after backfill DEFAULT 'demo' CHECK ∈ (demo/pilot/production) | |

**Preserved R-LC columns (13):** `ext_id`, `category`, `source`, `severity` (free-text — original R-LC), `title_en`, `title_ar`, `description_en`, `description_ar`, `affected_clause_categories`, `published_date`, `effective_date` (DATE — original R-LC), `compliance_deadline`, `is_seed`. Plus the audit cols + `is_active`. R-LC fn_'s continue to query `impact_signal` (now a view) without rewrite.

**Indexes (M7-added).** UNIQUE(tenant_id, dedup_hash) + `idx_osint_signal_tenant_fetched_at(tenant_id, fetched_at DESC)` + `idx_osint_signal_tenant_kind_severity(tenant_id, kind, severity_v2)` + `idx_osint_signal_tenant_source_event(tenant_id, source_id, event_date_v2 DESC)` + `idx_osint_signal_geographies_gin USING GIN(geographies jsonb_path_ops)` + `idx_osint_signal_affected_entities_gin USING GIN(affected_entities jsonb_path_ops)` + `idx_osint_signal_osint_source_id ON osint_signal(osint_source_id) WHERE osint_source_id IS NOT NULL` (Stage 2 S2-7 patch — partial WHERE excludes the 17 R-LC backfill rows).

**Existing R-LC indexes** (`idx_impact_signal_category`, `_published`, `_severity`) auto-renamed by RENAME — preserved as-is.

**FK auto-redirect (verified live).** `impact_signal_contract.signal_id_fkey` was REFERENCES `impact_signal(id)`; PostgreSQL handles the RENAME automatically — post-migration the FK now references `osint_signal(id)` ON DELETE CASCADE (verified via `pg_get_constraintdef` on both branches).

**Backfill (migration 104 step 4).** 17 R-LC rows (db-design said 18 — DEV-M7-1 non-blocking; UPDATE pattern is count-agnostic via `WHERE tenant_id IS NULL`): `signal_kind_subtype='manual_curated'`, `kind` derived from R-LC `category` mapping, `severity_v2` derived from `severity` mapping, `source_reliability=1.00` (manual curated baseline), `fetched_at=COALESCE(updated_at, created_at)`, `event_date_v2=effective_date::timestamptz`, `title=title_en`, `summary=description_en`, `source_id='manual_curated'`, `raw_payload=jsonb_build_object(<all original R-LC fields>)`, `dedup_hash=sha256('manual_curated|<date>|<lower(trim(title_en))>')`, `data_classification='demo'`.

---

### 5. `source_health`

**Purpose.** Per-source health snapshot. One row per `(tenant_id, osint_source_id)`. Updated by 5-min health-check cron via `fn_source_health_record`.
**Owned by:** M7. **Audit trigger:** **OFF (Q-NEW6 / OQ-6 lock)** — 5-min × 8 sources × N tenants would flood audit_log; the table's own `checked_at + state` columns are its ledger. **No `created_by` / `updated_by` columns** — cron has no actor. **RLS:** FORCE; 2 policies (tenant-scope SELECT via `source.read` + deny-direct-modify RESTRICTIVE; writes only via DEFINER `fn_source_health_record`).
...
| `tenant_id` | UUID | NOT NULL REFERENCES tenant(id) ON DELETE RESTRICT | |
| `osint_source_id` | BIGINT | NOT NULL REFERENCES osint_source(id) ON DELETE CASCADE | |
| `state` | TEXT | NOT NULL CHECK ∈ (healthy/degraded/failing/unauthorised) | |
| `last_success_at` | TIMESTAMPTZ | | Set when state transitions to/remains `healthy`. |
| `last_failure_at` | TIMESTAMPTZ | | Set when state transitions to/remains in failure family. |
| `last_error_message` | TEXT | | **[SENSITIVE]** Truncated to 500 chars by fn_. Listed in project.config.json sensitiveFields + fn_audit_trigger redact list (defence-in-depth though source_health audit trigger is OFF). |
| `signals_24h` | INTEGER | NOT NULL DEFAULT 0 | Rolling 24h count maintained by fn_source_health_record on every cron tick. |
| `checked_at` | TIMESTAMPTZ | NOT NULL DEFAULT now() | |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |
| `created_at` / `updated_at` | TIMESTAMPTZ | NOT NULL DEFAULT now() | |

**Indexes.** PK + UNIQUE(tenant_id, osint_source_id) + `idx_source_health_tenant_id` + `idx_source_health_osint_source_id` + `idx_source_health_tenant_state(tenant_id, state)`.

---

## View

### `impact_signal` — backward-compat shim (Q1 + OQ-7 lock)

**Purpose.** Trivial column projection over `osint_signal` exposing the original 19 R-LC columns. R-LC fn_'s (`fn_impact_signal_list` / `_get` / `_bulk_amend` / `_mark_reviewed` / `_notify_drafters`) read this view without rewrite (AC-S2-03 + AC-S2-05).

```sql
CREATE OR REPLACE VIEW impact_signal AS
SELECT id, ext_id, category, source, severity, title_en, title_ar,
       description_en, description_ar, affected_clause_categories,
       published_date, effective_date, compliance_deadline, is_seed,
       created_at, updated_at, created_by, updated_by, is_active
FROM osint_signal;
```

**RLS inheritance.** Views inherit the underlying table's RLS by default. R-LC users see only tenant-scoped rows via `osint_signal_tenant_isolation_select` policy (which includes `signal.read.all` OR `contract.read.department` OR `contract.edit` permission checks for back-compat). New OSINT signals leave `title_en` / `title_ar` / `description_en` / `description_ar` NULL and so are naturally excluded from R-LC ILIKE search results — desired semantics (R-LC UI renders only manual-curated signals).

---

## Functions (12 new)

### Reads (5; all SECURITY INVOKER STABLE)

| Function | Permission gate | Purpose |
|---|---|---|
| `fn_tenant_get_current(BIGINT)` | authenticated | Returns tenant row matching `app.current_tenant_id` GUC; NULL if unset. Used by rls.middleware to validate tenantId claim. JSONB: `{id, slug, displayName, configPack}`. |
| `fn_osint_source_list(BIGINT, JSONB, INTEGER, INTEGER)` | `source.read` | Paginated source list with embedded health badge per row (LEFT JOIN source_health on osint_source_id + tenant_id). Filterable by kind / state / search (ILIKE on source_id OR display_name). Sort: display_name ASC. credentialRef NEVER projected. JSONB: `{data: OsintSourceListItem[], pagination: PaginationMeta}`. |
| `fn_osint_source_get_by_id(BIGINT, BIGINT)` | `source.read` | Single source detail with embedded health + credential metadata (kind + lastRotatedAt only — credentialRef NEVER projected per AC-S3-04). 22023 → 404 if not found for tenant. |
| `fn_osint_signal_list(BIGINT, JSONB, INTEGER, INTEGER)` | `signal.read.all` | Paginated signal list. Filters: kind / sourceId (TEXT denorm) / severityMin (ordered: informational(1) < low(2) < medium(3) < high(4) < critical(5)) / since (fetched_at >= ISO 8601) / geographyIntersects (ISO 3166-1 alpha-2 via @> on geographies JSONB) / affectedEntityId (via @> on affected_entities JSONB). Sort: event_date_v2 DESC NULLS LAST, fetched_at DESC. RLS auto-scopes tenant. |
| `fn_source_health_list(BIGINT)` | `source.read` | Source health monitor — joins source_health with osint_source (display_name + kind). Returns BARE ARRAY (S2-12 documented design exception — bounded set). Sort: failing > unauthorised > degraded > healthy, then display_name. |

### Writes — SECURITY INVOKER (4)

| Function | Permission gate | Purpose |
|---|---|---|
| `fn_osint_source_create(BIGINT, JSONB)` | `source.manage` | Creates source row for current tenant. Validates kind / format / refreshSeconds (≥60) / sourceReliability ([0,1]). 23505 on duplicate (tenant_id, source_id). Returns full detail via chained `fn_osint_source_get_by_id`. |
| `fn_osint_source_update(BIGINT, BIGINT, JSONB)` | `source.manage` | Partial update. sourceId immutable — payload containing it raises 22023. S2-23 FK pre-validation: 22023 → 404 if source not found for tenant. |
| `fn_osint_source_delete(BIGINT, BIGINT)` | `source.manage` | Soft-delete (is_active=false AND enabled=false). Existing osint_signal rows referencing the source remain queryable. Returns `{id, deactivated: true, message}`. |
| `fn_osint_source_test_pull(BIGINT, BIGINT)` | `source.manage` | Emits pg_notify(`osint_test_pull`); BE worker LISTENs and dispatches manual fetch within ~10s. Returns 202. 22023 → 409 if source.enabled=false ('Source is disabled — enable before test-pull'). |

### Writes — SECURITY DEFINER (3)

| Function | Permission gate | Purpose | Grant |
|---|---|---|---|
| `fn_source_credential_set(BIGINT, BIGINT, TEXT, TEXT)` | `source.manage` (defence-in-depth at fn body line 1) | Upserts credential indirection. credentialRef MUST match `/^(env:|vault:)/`; plain-text rejected via heuristic. Returns `{id, credentialKind, lastRotatedAt}` — credentialRef NEVER returned. | GRANT TO neondb_owner |
| `fn_osint_signal_upsert(JSONB)` | system-only | Idempotent upsert via UNIQUE(tenant_id, dedup_hash). Emits pg_notify(`osint_signal_inserted`) ONLY on the insert path (not on conflict). S2-20 actor sentinel coercion (v_actor=0 → NULL). Synthesises R-LC compat columns (ext_id, category, source, severity, title_en, published_date) so NOT-NULL R-LC columns don't break new OSINT inserts. | **NO role grant** — only neondb_owner connection (worker pool) can EXECUTE. AC-S7-05. |
| `fn_source_health_record(BIGINT, TEXT, TEXT, INTEGER)` | system-only | Upserts source_health from cron worker. last_error_message truncated to 500 chars. last_success_at / last_failure_at toggled by state transition. | **NO role grant** — cron-only. |

### `fn_audit_trigger` — extended (migration 102; redact list 27 → 30 names)

`v_redact_fields` now includes `credential_ref`, `raw_payload`, `last_error_message` in addition to the M0..M4 names. Body otherwise byte-for-byte identical to migration 041. Per `feedback_fn_rewrites_lose_safety_guards.md`, the migration explicitly re-applies the trio after CREATE OR REPLACE: `COMMENT ON FUNCTION` + `REVOKE EXECUTE ... FROM PUBLIC` + `GRANT EXECUTE ... TO neondb_owner`. **Stage 2 round 1 caught the missing trio** — patched in 3 SQL lines before DB Implementation ran.

---

## Audit triggers

**Created (3):** `audit_osint_source_changes` ON osint_source; `audit_source_credential_changes` ON source_credential; `audit_osint_signal_changes` ON osint_signal (renamed from `audit_impact_signal_changes`).

**Deliberately OFF (2):**

- `tenant` — CC1 / OQ-8 lock. UUID PK incompatible with `audit_log.record_id` BIGINT. Defer to CR-C hash-chained audit_log replacement.
- `source_health` — Q-NEW6 / OQ-6 lock. 5-min × 8 sources × N tenants ≈ 2,880 audit rows/day from health checks alone — would dominate audit_log signal-to-noise. The table's own `checked_at + state` columns are its ledger.

---

## RLS policies (13 across 5 tables)

| Table | Policies |
|---|---|
| `tenant` | `tenant_self_row_select` (SELECT via app.current_tenant_id = id), `tenant_platform_admin_all` (ALL via fn_current_user_has_permission('user.manage')) |
| `osint_source` | `osint_source_tenant_select` (SELECT via tenant scope + source.read), `osint_source_tenant_modify` (ALL via tenant scope + source.manage), `osint_source_deny_direct_delete` (RESTRICTIVE FOR DELETE USING false) |
| `source_credential` | `source_credential_tenant_manage_only` (ALL via tenant scope + source.manage), `source_credential_deny_direct_delete` (RESTRICTIVE FOR DELETE USING false) |
| `osint_signal` | `osint_signal_tenant_isolation_select` (SELECT via tenant scope + signal.read.all OR contract.read.department OR contract.edit), `osint_signal_deny_direct_insert` / `_update` / `_delete` (RESTRICTIVE — writes only via DEFINER fn_osint_signal_upsert) |
| `source_health` | `source_health_tenant_select` (SELECT via tenant scope + source.read), `source_health_deny_direct_modify` (RESTRICTIVE — writes only via DEFINER fn_source_health_record) |

---

## pg_notify channels (2 new — neither exposed via HTTP)

| Channel | Emitted by | Payload | Consumer |
|---|---|---|---|
| `osint_signal_inserted` | `fn_osint_signal_upsert` (DEFINER, system-only) — ONLY on the insert path (not on conflict) | `{id, tenantId, sourceId, severity, kind}` | CR-E rule engine (future module); audit / observability tap (future) |
| `osint_test_pull` | `fn_osint_source_test_pull` (INVOKER, source.manage) | `{osintSourceId, tenantId, sourceId, actorId, requestedAt}` | BE fetch worker (S7) — registered LISTEN at startup |

**F-3 production caveat:** Neon `-pooler` (PgBouncer transaction pooling) drops session-level LISTEN. Worker connections MUST use Neon **direct** endpoint — see ops-runbook.

---

## Permissions seeded (4 net-new + 9 role grants — migration 100)

| Permission code | Granted to (roles) |
|---|---|
| `source.read` | Super Admin, platform_admin, executive |
| `source.manage` | Super Admin, platform_admin |
| `signal.read.all` | Super Admin, platform_admin, executive, legal_counsel |
| `osint.signal.upsert` | (no role grant — reserved for system DEFINER plumbing) |

`compliance_esg` grant guarded by `WHERE EXISTS` no-op — role does not exist in CR-A; defer to a future CR that adds it.

---

## PUBLIC EXECUTE invariant — S2-21 sustained at the M3 set of 5 (seventh consecutive clean module)

M7 contributes **0 net-new PUBLIC EXECUTE grants**. The full allowlist remains exactly 5 (all from M3): `fn_signature_get_by_invitation_token`, `fn_signature_sign`, `fn_signature_decline`, `fn_signer_qa_session_start`, `fn_signer_qa_session_record_message`. All 12 M7 fn_'s end with the explicit `REVOKE FROM PUBLIC` trio per migration 107.

---

## Key DB Impl outcomes (M7)

| ID | Severity | Topic | Outcome |
|---|---|---|---|
| **DEV-M7-1** | non-blocking | db-design.md says 18 R-LC rows; live count is 17 | Back-fill UPDATE pattern is count-agnostic (`WHERE tenant_id IS NULL`). All 17 active impact_signal rows touched correctly. AC-S2-02 verifies via signal_kind_subtype tag, not exact count. Recommended fix: update db-design.md to count-agnostic phrasing post-CR-A. |
| **M7-DBI-INFO-1** | INFO | S2-21 sustained at 5 | Seventh-consecutive-clean module. PUBLIC EXECUTE grants verified live on both branches via `pg_proc.proacl` enumeration. |
| **M7-DBI-INFO-2** | INFO | FK auto-redirect verified | `impact_signal_contract.signal_id_fkey` auto-redirected from `impact_signal(id)` to `osint_signal(id)` post-RENAME on both branches (verified via `pg_get_constraintdef`). |
| **M7-DBI-INFO-3** | INFO | Stage 2 round 1 caught 2 surgical issues | S2-7 missing FK btree on osint_signal.osint_source_id + S2-21/B14 missing REVOKE/GRANT trio on fn_audit_trigger CREATE OR REPLACE. Both patched in 3 SQL lines (Migration File Plan row updates + body edits) before DB Implementation ran. Stage 2 round 2 PASS first-run after the patches. |
| **F-1** | CRITICAL — caught at Testing Agent | GDELT 2-letter ISO vs 3-letter CAMEO mismatch | Production fetcher would have dropped ALL rows because the country filter would never match. Adapter now converts CAMEO 3-letter → ISO 2-letter via static mapping table; geographyFilter check operates on the ISO form post-conversion. |
| **F-3** | INFO — infrastructure | pg_notify untestable on Neon -pooler | Production worker connection MUST use Neon direct endpoint, not -pooler (PgBouncer transaction pooling drops session-level LISTEN). Documented in ops-runbook M7 section. |
