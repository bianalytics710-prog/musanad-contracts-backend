...
# M7 — OSINT Source Framework + Adapter Protocol (CR-A; CRIP Wave 1 chassis)

> **Module:** M7 — OSINT chassis for the CRIP enhancement layer
> **Generated:** 2026-05-09
> **Migrations:** 100..107 (8 files; 0 impl-time patches)
> **Schema version:** `schema_migrations.version = 107` on both `m0-foundation` + `test` branches
> **OpenAPI:** bumped `0.9.0` → `1.0.0` (CR-A is the chassis CR for the CRIP enhancement layer; semantic-major bump justified)
> **Audience:** Developers extending the OSINT layer (adding new adapters, debugging the fetch worker, wiring downstream CRs B/C/D/E/F/G/H/I)

---

## 1. What M7 ships

M7 is the **OSINT chassis** — admin source registry + adapter protocol + 8 live free-source adapters + 2 in-process workers + admin UI. **5 new tables** (tenant minimal, osint_source, source_credential, osint_signal *via rename+extend from impact_signal*, source_health). **12 new fn_'s** all owned by neondb_owner (zero net-new PUBLIC; baseline-5 set unchanged for the **seventh consecutive clean module**). **9 new HTTP endpoints**, all JWT-authed. **2 new pg_notify channels** (`osint_signal_inserted` for the future CR-E rule engine; `osint_test_pull` for the in-process fetch worker LISTEN). **First multi-tenancy footprint in the system** — ADNOC seed UUID `00000000-0000-0000-0000-000000000001` is the lone tenant in CR-A; CR-C extends.

## 2. Routes

### Admin source registry (`/admin/sources`)

| Method | Path | fn_ | Permission | Rate limit | Notes |
|---|---|---|---|---|---|
| GET | `/api/v1/admin/sources` | `fn_osint_source_list` | `source.read` | authedRead | Paginated; filterable by `kind` / `state` / `search`; embedded health badge per row; credentialRef NEVER projected. |
| POST | `/api/v1/admin/sources` | `fn_osint_source_create` | `source.manage` | authedWrite | Returns 201; sourceId immutable post-create; tenant_id derived from `app.current_tenant_id` GUC. |
| GET | `/api/v1/admin/sources/:id` | `fn_osint_source_get_by_id` | `source.read` | authedRead | Embeds health badge + credential metadata (kind + lastRotatedAt only — credentialRef NEVER projected per AC-S3-04). |
| PATCH | `/api/v1/admin/sources/:id` | `fn_osint_source_update` | `source.manage` | authedWrite | Partial update; payload containing `sourceId` rejected with 400 + field='sourceId' (AC-S3-08). |
| DELETE | `/api/v1/admin/sources/:id` | `fn_osint_source_delete` | `source.manage` | authedWrite | Soft-delete (is_active=false AND enabled=false); existing osint_signal rows referencing the source remain queryable (AC-S3-07). |
| POST | `/api/v1/admin/sources/:id/test-pull` | `fn_osint_source_test_pull` | `source.manage` | authedWrite | **202 Accepted** — emits pg_notify(osint_test_pull); BE worker LISTENs and dispatches the actual fetch within ~10s. 409 if `enabled=false`. |
| POST | `/api/v1/admin/sources/:id/credential` | `fn_source_credential_set` | `source.manage` | authedWrite | DEFINER carve-out (defence-in-depth gate inside fn body). credentialRef MUST match `/^(env:|vault:)/`; plain-text rejected. credentialRef NEVER returned. |

### Signals (`/signals`)

| Method | Path | fn_ | Permission | Notes |
|---|---|---|---|---|
| GET | `/api/v1/signals` | `fn_osint_signal_list` | `signal.read.all` | Paginated; filters: `kind` / `sourceId` (TEXT denorm) / `severityMin` (ordered) / `since` / `geographyIntersects` (ISO 3166-1 alpha-2) / `affectedEntityId`. Order: event_date DESC NULLS LAST, fetched_at DESC. `rawPayload` IS in the response (legal_counsel needs SDN entry text — CR-H consumer); redacted only at log + audit layer. |

### Source health (`/admin/source-health`)

| Method | Path | fn_ | Permission | Notes |
|---|---|---|---|---|
| GET | `/api/v1/admin/source-health` | `fn_source_health_list` | `source.read` | Returns a **bare array** (S2-12 documented design exception — bounded set per tenant; ~14 rows max in CR-A). Sort: failing > unauthorised > degraded > healthy, then displayName. |

## 3. Adapter protocol

**File:** `src/adapters/source-adapter.ts` — exports the base `SourceAdapter` interface (snake_case verbatim from the brief Backend section per AC-S4-01 — the rest of the project is camelCase, but the protocol contract is intentionally Python-style snake_case for cross-language reference parity). Helpers: `computeDedupHash(sourceId, eventDate, title) → SHA-256 hex` and `resolveCredential(credentialRef) → string | null` (resolves `env:VARNAME` against `process.env`; rejects plain-text and unknown schemes).

Every adapter extends or implements this and is keyed by `source_id` matching `osint_source.source_id`:

| source_id | Class | File | Reliability | refresh_seconds | Notes |
|---|---|---|---|---|---|
| `ofac_sdn` | OfacSdnAdapter | `ofac-sdn.adapter.ts` | 1.0 | 86400 | XML; CAATSA / NSPM-25 → severity=critical via severity_mapping.programContains rules |
| `eu_consolidated` | EuConsolidatedAdapter | `eu-consolidated.adapter.ts` | 1.0 | 86400 | XML; extends `xml-sanctions-base.adapter.ts` |
| `un_security_council` | UnSecurityCouncilAdapter | `un-security-council.adapter.ts` | 1.0 | 86400 | XML; same base |
| `uk_hmt` | UkHmtAdapter | `uk-hmt.adapter.ts` | 1.0 | 86400 | XML; same base |
| `rss_<slug>` × 6 | RssAdapter | `rss-aggregator.adapter.ts` | 0.80–0.95 | 900 | Multi-feed; per-feed fault isolation; severity via titleContains rules |
| `commodity_crude` | CommodityCrudeAdapter | `commodity-crude.adapter.ts` | 0.90 | 300 | JSON via `oilpriceapi.com` (Q-NEW1); per-marker BRENT/DUBAI/MURBAN/WTI; severity via absChangePctGte |
| `gdelt_v2` | GdeltAdapter | `gdelt-v2.adapter.ts` | 0.65 | 900 | CSV via `lastupdate.txt` index → `.export.CSV.zip`; ADNOC relevance filter via geography_filter; **CAMEO 3-letter → ISO 2-letter conversion** (F-1 fix — production-blocking issue caught by Testing Agent) |
| `fx_usd_aed` | FxAdapter | `fx-usd-aed.adapter.ts` | 0.85 | 300 | JSON via `exchangerate.host` primary + `open.er-api.com` fallback (Q-NEW2); AED peg deviation severity via pegDeviationPctGte |

## 4. Workers

Both workers live in `src/workers/` and are wired in `src/server.ts` after route mounting; both default-disabled and no-op in `NODE_ENV=test`.

### `src/workers/source-fetch.worker.ts`

- **Cadence:** every 1 minute (configurable via `SOURCE_FETCH_CRON`)
- **Env flag:** `SOURCE_FETCH_WORKER_ENABLED=true` to start
- **Logic:** on each tick, query active enabled sources for the configured tenant(s); for each source whose `(now - last_fetched_at) >= refresh_seconds`, dispatch the corresponding adapter; for each `RawSignal` yielded by `adapter.fetch(since)`, call `adapter.normalise()` then invoke `fn_osint_signal_upsert(payload)` via the **system DB connection (neondb_owner pool)** — there is no role grant on `fn_osint_signal_upsert` so only the owner can EXECUTE.
- **Per-source dispatch loop:** wrapped in try/catch — a single adapter failure must NOT abort the per-tenant tick (BE_02 fault isolation rationale comments inline).
- **LISTEN side:** registers `LISTEN osint_test_pull` at startup; on notify, dispatches a manual fetch for the named source within ~10s (AC-S7-06). **Production caveat (F-3):** Neon `-pooler` PgBouncer transaction pooling drops session-level LISTEN. Production worker connection MUST use Neon **direct** endpoint, not `-pooler`. See ops-runbook M7 section.

### `src/workers/source-health.worker.ts`

- **Cadence:** every 5 minutes (configurable via `SOURCE_HEALTH_CRON`)
- **Env flag:** `SOURCE_HEALTH_WORKER_ENABLED=true` to start
- **Logic:** on each tick, for each active source, call `adapter.health_check()` → `{ state, error? }`; compute `signals_24h` rolling counter via `SELECT count(*) FROM osint_signal WHERE osint_source_id = ? AND fetched_at > now() - interval '24 hours'`; invoke `fn_source_health_record(p_osint_source_id, p_state, p_last_error_message, p_signals_24h)` (DEFINER + REVOKE-only; only neondb_owner can EXECUTE).

## 5. Environment variables

| Variable | Required | Default | Purpose |
|---|---|---|---|
| `SOURCE_FETCH_WORKER_ENABLED` | optional | `false` | Start the 1-min fetch worker on boot. Must be `true` in production. |
| `SOURCE_FETCH_CRON` | optional | `* * * * *` (1-min) | Override the fetch tick cadence. |
| `SOURCE_HEALTH_WORKER_ENABLED` | optional | `false` | Start the 5-min health-check worker on boot. Must be `true` in production. |
| `SOURCE_HEALTH_CRON` | optional | `*/5 * * * *` (5-min) | Override the health-check tick cadence. |
| `COMMODITY_API_KEY` | optional in CR-A | n/a | Resolved by `commodity-crude.adapter.ts` via `resolveCredential('env:COMMODITY_API_KEY')`. The CR-A oilpriceapi.com free tier does not strictly require a key for the public endpoints used; absence of this env var degrades the adapter's `health_check()` to `state=degraded` (does NOT block fetches). |
| `FX_API_KEY` | optional | n/a | `fx-usd-aed.adapter.ts` — same pattern; primary `exchangerate.host` works without a key in CR-A. |

The `source_credential` table seeds `env:COMMODITY_API_KEY` and `env:FX_API_KEY` placeholders (migration 106). `resolveCredential()` reads `process.env.COMMODITY_API_KEY` etc. at adapter dispatch time — never persists the resolved value.

## 6. How to add a new adapter

1. **Create the adapter class** — `src/adapters/<source-id>.adapter.ts` implementing `SourceAdapter`. Either extend `xml-sanctions-base.adapter.ts` (for XML sources sharing the OFAC/EU/UN/UK shape) or implement from scratch.
2. **Register in the worker dispatcher** — `src/workers/source-fetch.worker.ts` keys adapters by `source_id`. Add the new adapter to the registry map.
3. **Seed the source row** — add an INSERT to a new migration `database/migrations/NNN_<source-id>_seed.sql` with `ON CONFLICT(tenant_id, source_id) DO NOTHING`.
4. **Seed the credential** — if needed, INSERT into `source_credential` with `credential_ref = 'env:<VARNAME>'`. Document the env var in `.env.example`.
5. **Run migrations + verify** — Stage 2 column-existence check: every column referenced by the adapter MUST exist on `osint_signal` (Section 1.4). The `dedup_hash` algorithm is canonical: `sha256(source_id || '|' || isoformat(event_date OR fetched_at) || '|' || lower(trim(title)))`.
6. **Write adapter unit tests** — `tests/adapters/<source-id>.adapter.test.ts` with at least 3 tests: parse + normalise + edge-case (CAATSA-equivalent severity escalation OR rate-limit handling OR missing-credential health degradation).

## 7. Open issues / forward-looking

- **OPEN-BE-1** (LOW) — adapters declare `rate_limit` config but the worker does not yet enforce a token-bucket. `RateLimiterMemory` is already a dep; wire it before pilot traffic ramp.
- **OPEN-BE-2** (LOW) — GDELT 2.0 export endpoint serves `.export.CSV.zip`. Current adapter reads as text — works for fixture-driven tests but production needs a zip extractor.
- **OPEN-BE-3** (MEDIUM) — multi-replica leader election. Both workers run per-replica today. Per-tenant + per-source rate-limit drift is acceptable in single-tenant CR-A; pg_advisory_lock shim documented as the upgrade path for CR-C multi-tenant pilot.
- **F-1** (RESOLVED in CR-A) — GDELT CAMEO 3-letter vs ISO 2-letter mismatch caught by Testing Agent before push. Adapter now converts CAMEO → ISO via static mapping table.
- **F-3** (INFO) — `pg_notify` on Neon `-pooler` is unreliable. **Production worker connections MUST use Neon direct endpoint.** See ops-runbook.
- **DEV-M7-1** (non-blocking) — db-design.md says "18 R-LC rows"; live count is 17. Update db-design.md to count-agnostic phrasing post-CR-A.
- **FE-M7-001** (LOW, defer to CR-G) — sidebar visibility for executive role.
- **FE-M7-002** (MEDIUM) — test-pull delta polling (AC-S10-04 — 2s/30s burst poll).
