-- MIGRATION: 556_renewal_alert_send_drop_tenant_filter.sql
-- Date: 2026-06-04
-- Description:
--   DEFECT-CR-EXP-2 / SMOKE: fn_contract_renewal_alert_send (mig 555)
--   filtered `WHERE c.tenant_id = v_tenant_id` but the contract table has
--   no tenant_id column — RLS policies enforce tenant isolation via the
--   app.current_tenant_id GUC. Drop the explicit filter; the RLS policy
--   plus the GUC set inline at the controller already provides isolation.
--   (Matches the pattern in fn_dashboard_executive_expiring_contracts
--   which also relies on RLS, not an explicit column.)
...
BEGIN
  ...
  LOOP
    -- tenant isolation is enforced by the RLS policy on contract; the
    -- GUC v_tenant_id is set by the caller before invoking this fn.
...
COMMENT ON FUNCTION fn_contract_renewal_alert_send(BIGINT, JSONB, INTEGER, TEXT) IS
  'DEFINER. Patched mig 555 to drop the c.tenant_id filter — RLS on contract enforces tenant isolation.';
...
VALUES (556, '556_renewal_alert_send_drop_tenant_filter', NOW())
