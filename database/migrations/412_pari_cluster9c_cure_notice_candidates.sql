-- Migration: 412_pari_cluster9c_cure_notice_candidates.sql
-- Unit: Pari Procurement QA Phase 3.7 (2026-06-01) — Cluster 9c
-- Closes: Regression introduced by mig 408. Re-sorting the supplier scorecard ASC NULLS LAST
--         pushed Mubadala (score 64, with the only SLA breach in seed) out of the top-20
--         visible scorecard. The FE cure-notice block reads
--         `data.supplierRiskScorecard.some((r) => r.slaBreachCount180d > 0)` which is now false,
--         so the entire cure-notice affordance disappeared — and P19's intent was to surface
--         MORE candidates, not fewer.
-- Strategy: Expose a dedicated `cureNoticeCandidates` top-level array (any vendor with
--           supplier_breach_count_180d > 0, regardless of scorecard rank), so the FE can
--           render the section even when those vendors aren't in the top-20 scorecard slice.
...
BEGIN
  ...
  IF NOT (
    ...
    OR EXISTS (
      ...
      WHERE u.id = p_actor_id AND r.name IN ('Super Admin', 'platform_admin') AND u.is_active = TRUE
    )
  ) THEN
  ...
  supplier_risk AS (
    SELECT
      p.id AS counterparty_id, p.name_en AS counterparty_name, p.party_type,
      p.sanctions_status, p.icv_status, p.icv_pct, p.icv_last_checked,
      ...
      CASE WHEN AVG(lrs.health_score) < 50 THEN 'high'
    ...
    LEFT JOIN latest_risk_score lrs ON lrs.contract_id = co.id
      AND lrs.tenant_id = current_setting('app.current_tenant_id', true)::uuid
    WHERE p.is_active = TRUE
    GROUP BY p.id, p.name_en, p.party_type, p.sanctions_status, p.icv_status, p.icv_pct, p.icv_last_checked
  ),
  supplier_breach_count AS (
    SELECT co.counterparty_id,
      COUNT(*) FILTER (WHERE os.signal_kind_subtype IN ('sla_breach','milestone_slippage'))::integer AS sla_breach_count_180d
    FROM correlation c
    ...
    WHERE c.tenant_id = current_setting('app.current_tenant_id', true)::uuid
      AND c.is_active = TRUE AND c.created_at >= NOW() - 180 * INTERVAL '1 day'
      AND os.kind = 'internal'
    GROUP BY co.counterparty_id
  ),
  supplier_breach_window AS (
    SELECT co.counterparty_id, COUNT(*)::integer AS sla_breach_count_window
    FROM correlation c
    JOIN osint_signal os ON os.id = c.signal_id
    JOIN contract co ON co.id = c.contract_id
    WHERE c.tenant_id = current_setting('app.current_tenant_id', true)::uuid
      AND c.is_active = TRUE AND c.created_at >= NOW() - p_window_days * INTERVAL '1 day'
      AND os.kind = 'internal'
      AND os.signal_kind_subtype IN ('sla_breach','milestone_slippage')
    GROUP BY co.counterparty_id
  ),
  -- Cure-notice candidates: anchor on case-driven AND signal-driven sources
  -- so we surface vendors even when the scorecard top-20 doesn't include them.
  cure_candidates AS (
    SELECT DISTINCT p.id AS counterparty_id, p.name_en AS counterparty_name,
           sbc.sla_breach_count_180d
    FROM party p
    JOIN supplier_breach_count sbc ON sbc.counterparty_id = p.id
    WHERE p.is_active = TRUE
    UNION
    SELECT DISTINCT p.id, p.name_en, COALESCE(sbc.sla_breach_count_180d, 0)
    FROM party p
    JOIN contract co ON co.counterparty_id = p.id AND co.is_active = TRUE
    JOIN risk_case rc ON rc.contract_id = co.id
      AND rc.tenant_id = current_setting('app.current_tenant_id', true)::uuid
      AND rc.is_active = TRUE
      AND rc.status NOT IN ('approved','rejected','closed')
      AND rc.case_type IN ('sla_breach','correlation_alert')
    LEFT JOIN supplier_breach_count sbc ON sbc.counterparty_id = p.id
    WHERE p.is_active = TRUE
  ),
  primary_candidates AS (
    SELECT counterparty_id, counterparty_name, party_type, composite_risk_score, total_contract_value_aed
    FROM supplier_risk
    WHERE composite_risk_score IS NOT NULL AND composite_risk_score < 70
        ...
        FROM (
          SELECT jsonb_build_object(
            ...
            '_rank', ROW_NUMBER() OVER (
              ORDER BY ((alt2.counterparty_id + pc.counterparty_id) % 7) ASC,
  ...
  icv_per_contract AS (
    SELECT ca.contract_id,
      MAX(CASE WHEN ca.description ~ 'valid_until=\d{4}-\d{2}-\d{2}'
               THEN substring(ca.description from 'valid_until=(\d{4}-\d{2}-\d{2})')::date
               ELSE NULL END) AS valid_until
  ...
  icv_status_per_contract AS (
    SELECT co.id AS contract_id, co.counterparty_id, co.contract_number, co.value_aed, ipc.valid_until,
      CASE WHEN ipc.valid_until IS NULL THEN 'missing'
           ...
           ELSE 'up_to_date' END AS icv_status_derived
    FROM contract co
  ...
  financial_health AS (
    SELECT co.counterparty_id, p.name_en AS counterparty_name,
      os.signal_kind_subtype AS signal_kind, os.title AS signal_headline,
      os.fetched_at AS occurred_at, os.severity_v2 AS severity, os.url AS source_ref
  ...
  at_risk_counterparties AS (
    SELECT sr.counterparty_id FROM supplier_risk sr
    WHERE sr.composite_risk_score IS NOT NULL AND sr.composite_risk_score < 50
    UNION SELECT counterparty_id FROM supplier_breach_window
    UNION SELECT counterparty_id FROM financial_health
  ),
  kpi_current AS (
    SELECT
      ...
      (SELECT COUNT(DISTINCT counterparty_id) FROM icv_per_counterparty WHERE non_compliant_contracts > 0)::integer AS icv_non_compliant_count,
  ...
  kpi_prev_window AS (
    SELECT
      ...
      (SELECT COUNT(DISTINCT co.counterparty_id)::integer
         FROM correlation c JOIN osint_signal os ON os.id = c.signal_id
         JOIN contract co ON co.id = c.contract_id
        WHERE c.tenant_id = current_setting('app.current_tenant_id', true)::uuid AND c.is_active = TRUE
          AND c.created_at BETWEEN NOW() - 2 * p_window_days * INTERVAL '1 day' AND NOW() - p_window_days * INTERVAL '1 day'
          AND os.kind = 'internal' AND os.signal_kind_subtype IN ('sla_breach','milestone_slippage')
      ) AS supplier_breaches_count,
      (SELECT COUNT(DISTINCT counterparty_id) FROM icv_per_counterparty WHERE non_compliant_contracts > 0)::integer AS icv_non_compliant_count,
      (SELECT COUNT(DISTINCT co.counterparty_id)::integer
         FROM osint_signal os JOIN correlation c ON c.signal_id = os.id
         JOIN contract co ON co.id = c.contract_id
        WHERE os.tenant_id = current_setting('app.current_tenant_id', true)::uuid
          AND os.kind = 'news'
          AND os.signal_kind_subtype IN ('financial_distress','downgrade','default')
          AND os.fetched_at BETWEEN NOW() - 2 * p_window_days * INTERVAL '1 day' AND NOW() - p_window_days * INTERVAL '1 day'
      ) AS supplier_financial_distress_count,
      NULL::numeric(5,2) AS avg_supplier_risk_score
  ),
  concentration AS (
    SELECT sr.counterparty_id, sr.counterparty_name, sr.total_contract_value_aed,
      ROUND((sr.total_contract_value_aed * 100.0) / NULLIF((SELECT SUM(total_contract_value_aed) FROM supplier_risk), 0), 2) AS share_pct
    FROM supplier_risk sr
    WHERE sr.total_contract_value_aed > 0
    ORDER BY sr.total_contract_value_aed DESC LIMIT 8
  ),
  tier_distribution AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE risk_tier = 'low' AND composite_risk_score IS NOT NULL)::integer AS low_count,
      COUNT(*) FILTER (WHERE composite_risk_score IS NULL)::integer AS unscored_count
    FROM supplier_risk
  ),
  sla_trend_buckets AS (SELECT generate_series(0, 25) AS weeks_ago),
  sla_trend AS (
    SELECT stb.weeks_ago, (CURRENT_DATE - (stb.weeks_ago * 7))::date AS week_end,
      COALESCE((
        SELECT COUNT(*)::integer FROM correlation c
        JOIN osint_signal os ON os.id = c.signal_id
        WHERE c.tenant_id = current_setting('app.current_tenant_id', true)::uuid
          AND c.is_active = TRUE AND os.kind = 'internal'
          AND os.signal_kind_subtype IN ('sla_breach','milestone_slippage')
  ...
  SELECT jsonb_build_object(
    ...
    'supplierRiskScorecard', COALESCE((
      ...
      FROM (
        SELECT sr2.* FROM supplier_risk sr2
        ORDER BY sr2.composite_risk_score ASC NULLS LAST, sr2.total_contract_value_aed DESC
        LIMIT 20
      ) sr
    ...
    'icvComplianceTracker', COALESCE((
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'icvStatus', CASE WHEN ipc.missing_contracts > 0 THEN 'missing'
                            WHEN ipc.expired_contracts > 0 THEN 'expired'
                            WHEN ipc.expiring_contracts > 0 THEN 'expiring_within_90d'
                            ELSE 'compliant' END,
          'icvPct', CASE WHEN ipc.total_contracts = 0 THEN NULL
                         ELSE ROUND(((ipc.total_contracts - ipc.non_compliant_contracts) * 100.0) / ipc.total_contracts, 1) END,
        ...
        FROM icv_per_counterparty ipc JOIN party p ON p.id = ipc.counterparty_id
        WHERE ipc.non_compliant_contracts > 0
        ORDER BY ipc.non_compliant_value_aed DESC NULLS LAST LIMIT 15
      ) ranked
    ), '[]'::jsonb),
    ...
    -- NEW IN 412: dedicated cure-notice candidates list (P19)
    'cureNoticeCandidates', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'counterpartyId', cc.counterparty_id::text,
        'counterpartyName', cc.counterparty_name,
        'slaBreachCount180d', COALESCE(cc.sla_breach_count_180d, 0)
      ))
      FROM (SELECT DISTINCT * FROM cure_candidates ORDER BY sla_breach_count_180d DESC NULLS LAST LIMIT 10) cc
    ), '[]'::jsonb),
    'chartsData', jsonb_build_object(
      ...
      'tierDistribution', (SELECT jsonb_build_object(
        'high', td.high_count, 'medium', td.medium_count,
        'low', td.low_count, 'unscored', td.unscored_count
      ) FROM tier_distribution td),
      'slaTrendWeeks26', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'weekEnd', st.week_end, 'weeksAgo', st.weeks_ago, 'breachCount', st.breach_count
        ) ORDER BY st.weeks_ago DESC) FROM sla_trend st
      ), '[]'::jsonb)
    )
  ) INTO v_result;
...
COMMENT ON FUNCTION fn_dashboard_procurement_supplier_risk(bigint, integer)
  IS 'CR-G Procurement supplier-risk dashboard. v4 (mig 412): adds cureNoticeCandidates top-level
array (vendors with active SLA breach signals OR open risk cases of type sla_breach/correlation_alert
regardless of scorecard rank). Closes Pari P19 + the 408-induced regression where ASC NULLS LAST
sort hid the only SLA-breached vendor (Mubadala, score 64) out of the top-20 scorecard.';
...
VALUES (412, '412_pari_cluster9c_cure_notice_candidates', NOW())
