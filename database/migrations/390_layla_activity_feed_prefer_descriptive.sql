-- Migration: 390_layla_activity_feed_prefer_descriptive.sql
-- Unit: Layla Counsel QA Phase 3.7 (2026-05-31) — L4 finish
--
-- Many automatic activity rows (ai_summary_generated triggered by AI calls,
-- status_changed by state machine) have empty description_en. Sorting purely
-- by created_at brings these to the top and hides the narrative seeds.
-- Add a tier-zero priority for rows whose description is non-null/non-empty.
...
BEGIN
  ...
  -- L4 — Activity feed: prefer rows with non-empty description, then down-rank status_changed
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id',             a.id,
  ...
  RETURN jsonb_build_object(
    ...
    'regulatoryUpdates',    v_reg_updates_12w,
