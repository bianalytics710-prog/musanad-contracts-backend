-- Migration: 389_layla_fix_dashboard_obligations_cte.sql
-- Unit: Layla Counsel QA Phase 3.7 (2026-05-31) — fix mig 382 obligation CTE syntax
--
-- Mig 382 used ORDER BY inside a UNION-ALL leg (invalid in postgres) and a
-- bare alias `o` inside the JSON aggregate. Both caused fn_dashboard_legal_counsel
-- to error with "missing FROM-clause entry for table o". This rewrites the
-- obligations block cleanly.
...
BEGIN
  ...
  -- L7 — Diversified top-5 obligations: 1 per category + fill with overdue payments.
  -- Use a single ranked CTE with a window function instead of CTE UNION ALL with ORDER BY.
  WITH categorized AS (
    ...
    'top5', COALESCE((
      ...
      FROM (
        SELECT * FROM categorized
         WHERE rn = 1   -- 1-per-kind diversity
