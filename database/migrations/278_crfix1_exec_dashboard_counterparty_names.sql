-- ============================================================
-- Migration 278: CR-FIX1 Issue 2 — fn_dashboard_executive
--   Add counterpartyName / counterpartyNameAr / counterpartyEmirate
--   to topCounterpartiesByValue5 by JOINing party table.
--   Executive role lacks parties.read permission, so the FE cannot
--   make a separate /parties call — embed names directly in this fn.
-- ============================================================
...
COMMENT ON FUNCTION public.fn_dashboard_executive(integer) IS
  'CR-FIX1 (278): topCounterpartiesByValue5 now embeds counterpartyName/counterpartyNameAr/counterpartyEmirate via LEFT JOIN party — executive role lacks parties permission so names are embedded directly. DEBT-CRIJ-1: time-sensitive NOW() replaced with fn_demo_now() for demo time-freeze support (migration 244).';
...
VALUES (278, 'crfix1_exec_dashboard_counterparty_names', CURRENT_TIMESTAMP)
ON CONFLICT (version) DO NOTHING;

-- ============================================================
-- ROLLBACK (run manually if needed):
-- Revert to version without counterpartyName/counterpartyNameAr/counterpartyEmirate
-- in topCounterpartiesByValue5 — restore from migration 277 or prior snapshot.
-- DELETE FROM schema_migrations WHERE version = 278;
-- ============================================================
