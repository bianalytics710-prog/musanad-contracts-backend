-- Migration: 448_demo_scenario_trigger_ext_id_short.sql
-- Module: Demo Harness — DEBT-CRIJ-3 (cascade wiring, part 5)
-- Description: osint_signal.ext_id is varchar(60). The sha256-hex composed
--              in 446 produced strings up to ~82 chars and the INSERT
--              failed with 22001. Use a 16-hex-char prefix instead — still
--              uniquely identifies each scenario+title pair, fits in 60.
-- Date: 2026-06-01
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'demo_scenario % not found in tenant scope', p_scenario_id USING ERRCODE = 'P0002';
  END IF;

  IF NOT v_scenario.is_active THEN
    RAISE EXCEPTION 'demo_scenario % is inactive — cannot trigger', p_scenario_id USING ERRCODE = '23000';
  END IF;

  v_scenario_bigint := v_scenario.id;

  SELECT EXISTS (SELECT 1 FROM demo_seed_pack WHERE tenant_id = v_tenant_id AND pack_id = v_scenario.seed_pack_ref AND is_active = TRUE)
  INTO v_pack_exists;

  IF NOT v_pack_exists THEN
    RAISE EXCEPTION 'demo_seed_pack with pack_id % not found for scenario %', v_scenario.seed_pack_ref, p_scenario_id USING ERRCODE = 'P0002';
  END IF;
  ...
  IF p_scenario_id = 'trade_margin' THEN
    BEGIN
      SELECT fn_margin_recompute_for_price_change(p_actor_id, 'murban_osp', 110.75::NUMERIC, date_trunc('month', CURRENT_DATE)::date) INTO v_prepare_result;
    ...
    PERFORM fn_audit_log_record_v2('demo_scenario_run', v_run_id, 'INSERT', NULL,
      jsonb_build_object('actionCode', 'DEMO_SCENARIO_TRIGGER', 'scenarioId', p_scenario_id, 'prepared', TRUE, 'elapsedMs', v_elapsed_ms), NULLIF(p_actor_id, 0));
    RETURN jsonb_build_object('runId', v_run_id, 'elapsedMs', v_elapsed_ms, 'success', TRUE, 'prepared', TRUE, 'preparedAt', clock_timestamp(),
      'deepLink', '/app/finance/trade-positions', 'recomputeResult', v_prepare_result, 'expectedOutcomes', v_scenario.expected_outcomes,
  ...
  ELSIF p_scenario_id = 'labor_cascade' THEN
    ...
    WHERE tenant_id = v_tenant_id AND dedup_hash = 'mohre_labor|2024-08-30|federal decree-law no. 9 of 2024 — labor relations amendments';
    v_prepared := TRUE;
    v_elapsed_ms := (extract(epoch from clock_timestamp() - v_started_at) * 1000)::INTEGER;
    INSERT INTO demo_scenario_run (tenant_id, demo_scenario_id, triggered_by, triggered_at, outcome, success, elapsed_ms)
    ...
    PERFORM fn_audit_log_record_v2('demo_scenario_run', v_run_id, 'INSERT', NULL,
      jsonb_build_object('actionCode', 'DEMO_SCENARIO_TRIGGER', 'scenarioId', p_scenario_id, 'prepared', TRUE, 'elapsedMs', v_elapsed_ms), NULLIF(p_actor_id, 0));
    RETURN jsonb_build_object('runId', v_run_id, 'elapsedMs', v_elapsed_ms, 'success', TRUE, 'prepared', TRUE, 'preparedAt', clock_timestamp(),
      'deepLink', '/app/contracts?filter=labor_compliance', 'signalActive', TRUE, 'expectedOutcomes', v_scenario.expected_outcomes,
  ...
  ELSIF p_scenario_id = 'budget_burn' THEN
    ...
    PERFORM fn_audit_log_record_v2('demo_scenario_run', v_run_id, 'INSERT', NULL,
      jsonb_build_object('actionCode', 'DEMO_SCENARIO_TRIGGER', 'scenarioId', p_scenario_id, 'prepared', TRUE, 'static', TRUE, 'elapsedMs', v_elapsed_ms), NULLIF(p_actor_id, 0));
    RETURN jsonb_build_object('runId', v_run_id, 'elapsedMs', v_elapsed_ms, 'success', TRUE, 'prepared', TRUE, 'preparedAt', clock_timestamp(),
      'deepLink', '/app/contracts?filter=budget_burn', 'static', TRUE, 'expectedOutcomes', v_scenario.expected_outcomes,
  ...
  -- ext_id truncated to fit varchar(60) — 16-hex-char sha256 prefix is
  -- collision-safe for the small scenario+title fixture set.
  -- ============================================================
  ...
  FOR v_signal_item IN SELECT * FROM jsonb_array_elements(v_signals) LOOP
    ...
    WHERE tenant_id = v_tenant_id AND source_id = COALESCE(v_signal_item->>'sourceId', 'mock_social_x') LIMIT 1;
    ...
    IF v_src_id IS NOT NULL THEN
      v_ext_id := left(
        'demo:' || left(p_scenario_id, 12) || ':' ||
        substring(encode(digest((v_signal_item->>'title') || ':' || (v_signal_item->>'sourceId'), 'sha256'), 'hex') from 1 for 16),
        60
      );
      v_dedup_hash := left(
        COALESCE(v_signal_item->>'sourceId', 'mock_social_x')
        || '|' || to_char(CURRENT_DATE, 'YYYY-MM-DD')
        || '|' || lower(left(COALESCE(v_signal_item->>'title', p_scenario_id), 150)),
        250
      );
  ...
  PERFORM fn_audit_log_record_v2('demo_scenario_run', v_run_id, 'INSERT', NULL,
    jsonb_build_object('actionCode', 'DEMO_SCENARIO_TRIGGER', 'scenarioId', p_scenario_id, 'outcome', v_outcome, 'elapsedMs', v_elapsed_ms), NULLIF(p_actor_id, 0));
...
VALUES (448, '448_demo_scenario_trigger_ext_id_short', CURRENT_TIMESTAMP);

-- ============================================================
-- ROLLBACK
-- ============================================================
-- DELETE FROM schema_migrations WHERE version = 448;
-- -- Re-apply 446 to revert to the 82-char ext_id version (still broken).
-- ============================================================
