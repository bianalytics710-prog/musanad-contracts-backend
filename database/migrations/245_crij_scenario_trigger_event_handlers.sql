-- Migration: 245_crij_scenario_trigger_event_handlers.sql
-- Module: M17+M18 — CR-I + CR-J DEBT-CRIJ-3
-- Description: Rewrite fn_demo_scenario_trigger with per-scenario event-injection handlers.
--   Each CASE branch inserts a representative osint_signal + (where applicable) calls
--   fn_rule_evaluate_weather_fm_eligible or fn_advisory_draft_generate to produce real
--   correlation/advisory rows. Outcome JSONB now reflects actual counts, not zeros.
...
BEGIN
  ...
  CASE p_scenario_id
    ...
    -- ----------------------------------------------------------------
    -- hormuz: geopolitical signal via rss_lloyds_maritime source
    -- (rss_lloyds_maritime may not exist in demo seed; fall back to
    --  openweather which is always seeded. Correlations left to cron.)
    -- ----------------------------------------------------------------
    WHEN 'hormuz' THEN
      ...
      IF v_src_id IS NOT NULL THEN
        INSERT INTO osint_signal (
          ...
          title, summary, severity_v2, url, geographies,
          data_classification, is_active, created_at, updated_at
        ) VALUES (
          v_tenant_id, v_src_id, COALESCE(
            (SELECT source_id FROM osint_source WHERE id = v_src_id LIMIT 1), 'openweather'
          ),
      ...
      END IF;
      -- Correlations left to cron worker (no dedicated rule fn for geopolitical yet)
    ...
    WHEN 'ofac_sanctions' THEN
      ...
      IF v_src_id IS NOT NULL THEN
        INSERT INTO osint_signal (
          tenant_id, osint_source_id, source_id, kind, signal_kind_subtype,
          title, summary, severity_v2, url, affected_entities,
          data_classification, is_active, created_at, updated_at
        ) VALUES (
          v_tenant_id, v_src_id, COALESCE(
            (SELECT source_id FROM osint_source WHERE id = v_src_id LIMIT 1), 'rss_reuters_sanctions'
          ),
        ...
        RETURNING id INTO v_signal_id;
      END IF;
      -- No sanctions rule fn exists yet; correlations left to cron
    ...
    WHEN 'brent_review' THEN
      ...
      IF v_src_id IS NOT NULL THEN
        INSERT INTO osint_signal (
          tenant_id, osint_source_id, source_id, kind, signal_kind_subtype,
          title, summary, severity_v2, url, raw_payload,
          data_classification, is_active, created_at, updated_at
        ) VALUES (
          v_tenant_id, v_src_id, COALESCE(
            (SELECT source_id FROM osint_source WHERE id = v_src_id LIMIT 1), 'mock_social_x'
          ),
        ...
        RETURNING id INTO v_signal_id;
      END IF;
      -- No commodity rule fn exists yet; correlations left to cron

    -- ----------------------------------------------------------------
    -- epc_sla: internal milestone_slippage signal
    -- SKIP full correlation: needs specific seeded EPC contract_id.
    -- Insert signal only; log gap.
    ...
    -- ----------------------------------------------------------------
    -- renewal: calendar-driven lookahead — no signal kind fits cleanly.
    -- Emit an internal audit-style signal row only; log gap.
    ...
    WHEN 'renewal' THEN
      ...
      IF v_src_id IS NOT NULL THEN
        INSERT INTO osint_signal (
          ...
          v_tenant_id, v_src_id, 'mock_social_x',
    ...
    -- ----------------------------------------------------------------
    -- cyclone: weather signal + call fn_rule_evaluate_weather_fm_eligible
    -- This is the FULLY WIRED path.
    -- ----------------------------------------------------------------
    WHEN 'cyclone' THEN
      SELECT id INTO v_src_id FROM osint_source
      ...
      IF v_src_id IS NOT NULL THEN
        ...
        IF v_signal_id IS NOT NULL THEN
          ...
          -- If correlations exist, attempt advisory draft on first matched contract
    ...
    -- ----------------------------------------------------------------
    -- icv_shortfall: internal ICV signal
    -- Advisory draft attempted via icv_rectification_notice_v1
    -- GAP: fn_rule_evaluate_icv not yet implemented.
    --   Signal inserted; advisory draft requires a correlation row first.
    --   Insert a synthetic correlation then generate draft.
    -- ----------------------------------------------------------------
    WHEN 'icv_shortfall' THEN
      ...
      IF v_src_id IS NOT NULL THEN
        ...
        -- Synthetic correlation so advisory draft can be generated
        IF v_signal_id IS NOT NULL THEN
          SELECT c.id INTO v_first_contract_id
          FROM contract c
    ...
    -- ----------------------------------------------------------------
    -- esg_subcontractor: pick an existing ESG signal from mock_social_x
    -- (seeded in migration 241). Attempt advisory draft.
    -- ----------------------------------------------------------------
    WHEN 'esg_subcontractor' THEN
      -- Pick the newest existing ESG mock signal for this tenant
      ...
      IF v_esg_signal_id IS NULL THEN
        -- No pre-seeded signal; insert one now
        SELECT id INTO v_src_id FROM osint_source
        WHERE tenant_id = v_tenant_id AND source_id = 'mock_social_x' LIMIT 1;
        IF v_src_id IS NOT NULL THEN
          INSERT INTO osint_signal (
      ...
      -- Synthetic correlation + advisory draft for ESG
  ...
  -- ================================================================
  -- OUTCOME CAPTURE (window-snapshot from v_started_at)
  -- ================================================================
...
VALUES (245, '245_crij_scenario_trigger_event_handlers', CURRENT_TIMESTAMP)
ON CONFLICT (version) DO NOTHING;

-- ============================================================
-- ROLLBACK
-- ============================================================
-- DELETE FROM schema_migrations WHERE version = 245;
-- DROP FUNCTION IF EXISTS fn_demo_scenario_trigger(BIGINT, TEXT);
-- Restore prior version from migration 235 / 244 as needed.
-- ============================================================
