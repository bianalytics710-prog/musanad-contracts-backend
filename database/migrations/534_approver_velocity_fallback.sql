-- MIGRATION: 534_approver_velocity_fallback.sql
-- Date: 2026-06-03
-- Description:
--   Extend fn_dashboard_approver so the "Decision velocity" insights card
--   doesn't read empty for approvers whose most recent decision is more
--   than 30 days old. Fallback ladder: 30d → 90d → 180d → all-time.
--
--   Returns the actual window used as `decisionVelocityWindowDays` so the
--   FE can label the card honestly (e.g. "Decision velocity (last 90 days)").
...
BEGIN
  ...
  IF v_role IS NULL OR v_role NOT IN ('contract_approver', 'contract_approver_2', 'platform_admin', 'Super Admin') THEN
    RAISE EXCEPTION 'fn_dashboard_approver: forbidden' USING ERRCODE = '42501';
  END IF;
  ...
  -- Median decision time — rolling 30d, fallback to all-time.
  WITH rolling_curr AS (
    SELECT EXTRACT(EPOCH FROM (ad.decided_at - step.created_at))/3600.0 AS hours
     ...
     WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE
  ...
  -- ============================================================
  -- Insights — decision velocity with window fallback ladder.
  --   Pick the smallest window in {30, 90, 180, all-time} that contains
  --   at least one decision. All-time is implemented as "since the
  --   approver's first decision".
  -- ============================================================
  SELECT COUNT(*) INTO v_velocity_count
    FROM approval_decision ad
   WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE
     AND ad.decided_at >= fn_demo_now() - INTERVAL '30 days';
  IF v_velocity_count > 0 THEN
    v_velocity_window := 30;
  ELSE
    SELECT COUNT(*) INTO v_velocity_count
      FROM approval_decision ad
     WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE
       AND ad.decided_at >= fn_demo_now() - INTERVAL '90 days';
    IF v_velocity_count > 0 THEN
      v_velocity_window := 90;
    ELSE
      SELECT COUNT(*) INTO v_velocity_count
        FROM approval_decision ad
       WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE
         AND ad.decided_at >= fn_demo_now() - INTERVAL '180 days';
      IF v_velocity_count > 0 THEN
        v_velocity_window := 180;
      ELSE
        -- All-time — span from first decision to now, capped at 365 for
        -- chart density.
        SELECT LEAST(
          365,
          GREATEST(
            30,
            CEIL(EXTRACT(EPOCH FROM (fn_demo_now() - MIN(ad.decided_at)))/86400.0)::int
          )
        )
          INTO v_velocity_window
          FROM approval_decision ad
         WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE;
        v_velocity_window := COALESCE(v_velocity_window, 30);
      END IF;
    END IF;
  END IF;
  ...
  WITH days AS (
    SELECT generate_series(
      (fn_demo_now() - (v_velocity_window - 1 || ' days')::interval)::date,
      fn_demo_now()::date,
      INTERVAL '1 day'
    )::date AS day
  )
  ...
  LEFT JOIN (
    SELECT ad.decided_at::date AS day,
      ...
      JOIN approval_step step ON step.id = ad.approval_step_id
     WHERE ad.decided_by = v_user_id AND ad.is_active = TRUE
       AND ad.decided_at >= fn_demo_now() - (v_velocity_window || ' days')::interval
  ...
  -- ============================================================
  -- Insights — counterparty concentration
  -- ============================================================
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  -- ============================================================
  -- Insights — decision mix (last 90d)
  -- ============================================================
  SELECT jsonb_build_object(
  ...
  WHERE decided_by = v_user_id AND is_active = TRUE
  ...
  RETURN jsonb_build_object(
    ...
    'insights', jsonb_build_object(
      'decisionVelocity',          v_decision_velocity,
      'decisionVelocityWindowDays',v_velocity_window,
...
VALUES (534, 'approver_velocity_fallback', CURRENT_TIMESTAMP)
