-- ============================================================================
-- 068_approver_parity_r5_fix_nested_agg.sql
-- ============================================================================
-- Module:    M_parity (R5 — fix-up #2)
-- Owner:     Direct work
-- Depends:   067 (fn_dashboard_approver R5 + step.contract_id fix)
-- ----------------------------------------------------------------------------
-- DEFECT: 'aggregate function calls cannot be nested' on
-- decisionsByValue subquery — Postgres rejects SUM(...) inside
-- jsonb_agg(jsonb_build_object(...)) when the outer SELECT is itself a
-- GROUP BY scope.
--
-- FIX: split the aggregation into an inner subquery (GROUP BY bucket
-- with SUMs computed there), then jsonb_agg the pre-aggregated rows.
-- ----------------------------------------------------------------------------
...
VALUES (68, 'M_parity R5 fix: split decisionsByValue aggregate (no nested aggregates)', CURRENT_TIMESTAMP)
