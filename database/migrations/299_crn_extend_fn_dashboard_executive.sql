-- Migration: 299_crn_extend_fn_dashboard_executive.sql
...
--              'budgetBurnSummary' via inlined split-aggregate CTE. All 9 existing keys
--              (kpis, kpiPrev, trends, charts, lists, events14d, whatChangedToday,
--              recommendedActions, clausesTriggered) preserved byte-for-byte.
--              fn signature fn_dashboard_executive(integer) UNCHANGED.
--              Re-applies COMMENT + REVOKE PUBLIC + GRANT neondb_owner (B14/S2-21).
--              Base body sourced from migration 278 (CR-FIX1 exec dashboard).
...
BEGIN
  ...
  -- ============================================================
  -- CR-N ADDITIVE: budgetBurnSummary (10th top-level key)
  -- Split-aggregate CTEs: budget_by_contract / actual_by_contract / joined
  -- Reads new tables contract_budget + contract_cost_actual.
  -- Executive has finance.budget.read (granted in mig 300) — RLS permits.
  -- Self-contained: 4 scalar fields + top-3 list. Does NOT call fn_budget_burn_portfolio
  -- (that returns paginated detail; a 4-field inline CTE is lighter).
  -- ============================================================
  ...
  RETURN jsonb_build_object(
    ...
    -- CR-N: 10th additive key (budgetBurnSummary)
...
COMMENT ON FUNCTION public.fn_dashboard_executive(integer) IS
  'CR-N (299): Additive budgetBurnSummary key added as 10th top-level key. All 9 prior keys preserved byte-for-byte (kpis, kpiPrev, trends, charts, lists, events14d, whatChangedToday, recommendedActions, clausesTriggered). Base: CR-FIX1 mig 278.';
...
VALUES (299, '299_crn_extend_fn_dashboard_executive', CURRENT_TIMESTAMP)
...
-- ============================================================
-- Restore fn_dashboard_executive to mig 278 body (remove budgetBurnSummary key).
-- BEGIN;
-- -- [Paste migration 278 CREATE OR REPLACE here]
-- DELETE FROM schema_migrations WHERE version = 299;
-- COMMIT;
-- ============================================================
