-- Migration 077: R-LC6 patch — drop legacy fn_contract_list overloads.
--
-- Migration 076 created a new 21-param fn_contract_list with two extra
-- LEFT JOINs (party + user). The DROP FUNCTION at the top of 076 only
-- removed one specific overload; older 18-param (from M1c migration 022)
-- and 15-param (M1a 005) overloads still existed in pg_proc and Postgres
-- now sees TWO 21-param overloads (likely the 069 and 076 versions both
-- present due to a partial drop). The error
-- "fn_contract_list(...21 unknowns) is not unique" surfaces at every
-- contracts list call.
--
-- Fix: drop EVERY fn_contract_list overload by name, then recreate the
-- 21-param 076 body. This is safe because the controller only calls the
-- 21-param signature and no fn_ depends on the older overloads.
