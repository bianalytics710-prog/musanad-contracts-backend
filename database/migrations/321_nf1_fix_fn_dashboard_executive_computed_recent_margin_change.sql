-- Migration: 321_nf1_fix_fn_dashboard_executive_computed_recent_margin_change.sql
-- Module: CR-O NF-1 fix — fn_dashboard_executive recentMarginChange computed (not literal)
-- Description: Replaces the hardcoded deltaAed='-139040850.00' / deltaUsd='-37860000.00' literals
--              in the tradeMarginSummary.recentMarginChange block with a S2-24 split-aggregate CTE
--              that computes the true aggregate delta by comparing each open position's latest
--              price_change snapshot vs its immediately-prior snapshot via LAG() OVER (PARTITION BY
--              trade_position_id ORDER BY computed_at), summing deltas across positions whose most
--              recent price_change snapshot is within 30 days.  deltaUsd is summed from the
--              total_margin_usd column directly (each snapshot stores the FX-converted value at
--              compute time).  If NO recent price_change snapshot exists the block returns NULL
--              (same as the zero-data fallback) — never a frozen literal.
--
--              ALL other parts of fn_dashboard_executive are preserved byte-for-byte from
--              migration 316 (CR-O).  Function signature, STABLE marking, DECLARE block,
--              all 10 prior top-level keys, the defensive EXCEPTION wrapper, REVOKE/GRANT/COMMENT
--              trio — all unchanged.
--
...
COMMENT ON FUNCTION public.fn_dashboard_executive(integer) IS
  'NF-1 fix (321): tradeMarginSummary.recentMarginChange now COMPUTED via S2-24 split-aggregate CTEs (pc_ranked LAG+DISTINCT ON per position, recent_window 30d, delta_totals SUM). No hardcoded literals. Returns NULL gracefully when no price_change snapshot within 30 days. All 10 prior keys + tradeMarginSummary preserved byte-for-byte from mig 316 except the recentMarginChange subquery. CR-O (316): 11th key tradeMarginSummary. CR-N (299): 10th key budgetBurnSummary. kpis, kpiPrev, trends, charts, lists, events14d, whatChangedToday, recommendedActions, clausesTriggered all from prior migrations.';
...
VALUES (321, '321_nf1_fix_fn_dashboard_executive_computed_recent_margin_change', CURRENT_TIMESTAMP)
