-- MIGRATION: 519_tpa_fix_permission_fn.sql
-- Feature: TPA — switch fn_'s from non-existent fn_has_permission(bigint,text)
-- to the project-standard fn_current_user_has_permission(text).
-- Date: 2026-06-03

BEGIN;

-- We also need to set app.current_user_id for fn_current_user_has_permission
-- to work; the BE callFunction already does this via SET LOCAL when actorId
-- is passed. So we just swap the helper call.
...
BEGIN
  IF NOT fn_current_user_has_permission('tpa.review.read') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', p.id, 'playbookKey', p.playbook_key, 'agreementType', p.agreement_type,
    'nameEn', p.name_en, 'nameAr', p.name_ar, 'descriptionEn', p.description_en,
  ...
  SELECT jsonb_build_object(
    ...
    'clauses', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', c.id, 'clauseKey', c.clause_key,
        'clauseTitleEn', c.clause_title_en, 'clauseTitleAr', c.clause_title_ar,
        'criticality', c.criticality, 'displayOrder', c.display_order,
        'standardPosition', c.standard_position, 'fallbackPosition', c.fallback_position,
        'nonNegotiables', c.non_negotiables, 'redFlags', c.red_flags,
        'guidanceNotes', c.guidance_notes
      ) ORDER BY c.display_order, c.id)
      FROM tpa_playbook_clause c WHERE c.playbook_id = p.id AND c.is_active = TRUE
    ), '[]'::jsonb)
  ) INTO v_result
  FROM tpa_playbook p
...
CREATE OR REPLACE FUNCTION fn_tpa_review_create(
  p_actor_id BIGINT, p_playbook_id BIGINT, p_counterparty_name TEXT,
  p_counterparty_email TEXT, p_agreement_title TEXT, p_agreement_type TEXT
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE v_tenant_id UUID; v_id BIGINT; v_ref TEXT;
BEGIN
  v_tenant_id := current_setting('app.current_tenant_id', true)::uuid;
  IF NOT fn_current_user_has_permission('tpa.review.create') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM tpa_playbook WHERE id = p_playbook_id
                 AND tenant_id = v_tenant_id AND is_active = TRUE) THEN
    RAISE EXCEPTION 'tpa_playbook % not found', p_playbook_id USING ERRCODE = 'P0002';
  END IF;
  v_ref := 'TPA-' || to_char(NOW(), 'YYMM') || '-' ||
           lpad((COALESCE((
              SELECT MAX(NULLIF(regexp_replace(reference_code, '^TPA-\d{4}-', ''), '')::int)
              FROM tpa_review WHERE tenant_id = v_tenant_id
                AND reference_code LIKE 'TPA-' || to_char(NOW(), 'YYMM') || '-%'
           ), 0) + 1)::text, 4, '0');
  INSERT INTO tpa_review (tenant_id, playbook_id, reference_code,
    counterparty_name, counterparty_email, agreement_title, agreement_type, status,
    created_by, updated_by)
  VALUES (v_tenant_id, p_playbook_id, v_ref,
  ...
  RETURN jsonb_build_object('id', v_id, 'referenceCode', v_ref, 'status', 'pending_analysis');
END;
$fn$;
...
CREATE OR REPLACE FUNCTION fn_tpa_review_attach_document(
  p_actor_id BIGINT, p_review_id BIGINT, p_kind TEXT, p_file_name TEXT,
  p_mime TEXT, p_size_bytes INTEGER, p_storage_uri TEXT,
  p_page_count INTEGER, p_extraction_eng TEXT, p_sha256_hex TEXT
) RETURNS BIGINT LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE v_tenant_id UUID; v_id BIGINT;
BEGIN
  v_tenant_id := current_setting('app.current_tenant_id', true)::uuid;
  IF NOT (fn_current_user_has_permission('tpa.review.create')
       OR fn_current_user_has_permission('tpa.review.amend')) THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM tpa_review WHERE id = p_review_id
                 AND tenant_id = v_tenant_id AND is_active = TRUE) THEN
    RAISE EXCEPTION 'tpa_review % not found', p_review_id USING ERRCODE = 'P0002';
  END IF;
  INSERT INTO tpa_review_document (tenant_id, review_id, document_kind, file_name, mime_type,
    size_bytes, storage_uri, page_count, extraction_engine, sha256_hex, created_by)
  VALUES (v_tenant_id, p_review_id, p_kind, p_file_name, p_mime,
...
CREATE OR REPLACE FUNCTION fn_tpa_review_record_analysis(
  p_actor_id BIGINT, p_review_id BIGINT, p_findings JSONB,
  p_overall_verdict TEXT, p_overall_risk TEXT, p_risk_score INTEGER,
  p_executive_summary TEXT, p_model_version TEXT, p_prompt_hash TEXT
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE
  v_tenant_id UUID; v_finding JSONB;
  v_inserted INTEGER := 0; v_accept_n INTEGER := 0;
  v_amend_n INTEGER := 0; v_reject_n INTEGER := 0; v_conflict_n INTEGER := 0;
BEGIN
  v_tenant_id := current_setting('app.current_tenant_id', true)::uuid;
  IF NOT (fn_current_user_has_permission('tpa.review.create')
       OR fn_current_user_has_permission('tpa.review.amend')) THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM tpa_review WHERE id = p_review_id
    ...
    RAISE EXCEPTION 'tpa_review % not found', p_review_id USING ERRCODE = 'P0002';
  END IF;
  DELETE FROM tpa_review_finding WHERE review_id = p_review_id AND tenant_id = v_tenant_id;
  ...
  LOOP
    ...
    VALUES (v_tenant_id, p_review_id,
      ...
      v_finding->>'extractedText', v_finding->>'extractedLocation',
    ...
    CASE COALESCE(v_finding->>'aiVerdict', 'info')
      ...
      WHEN 'amend'  THEN v_amend_n  := v_amend_n  + 1; v_conflict_n := v_conflict_n + 1;
      WHEN 'reject' THEN v_reject_n := v_reject_n + 1; v_conflict_n := v_conflict_n + 1;
      ELSE NULL;
    END CASE;
  END LOOP;
  UPDATE tpa_review SET status = 'awaiting_review',
    overall_verdict = NULLIF(p_overall_verdict, ''),
    overall_risk = NULLIF(p_overall_risk, ''),
    risk_score = p_risk_score,
    accept_count = v_accept_n, amend_count = v_amend_n,
    reject_count = v_reject_n, conflict_count = v_conflict_n,
    executive_summary = p_executive_summary,
    llm_model_version = p_model_version, llm_prompt_hash = p_prompt_hash,
    llm_analysed_at = NOW(), llm_error = NULL,
    updated_at = NOW(), updated_by = p_actor_id
  WHERE id = p_review_id AND tenant_id = v_tenant_id;
  RETURN jsonb_build_object('reviewId', p_review_id, 'findingsCount', v_inserted,
    'acceptCount', v_accept_n, 'amendCount', v_amend_n, 'rejectCount', v_reject_n,
    'overallVerdict', p_overall_verdict, 'overallRisk', p_overall_risk);
END;
$fn$;
...
CREATE OR REPLACE FUNCTION fn_tpa_review_record_failure(p_actor_id BIGINT, p_review_id BIGINT, p_error TEXT)
...
BEGIN
  ...
  IF NOT (fn_current_user_has_permission('tpa.review.create')
    ...
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;
  UPDATE tpa_review SET status = 'failed', llm_error = p_error,
    updated_at = NOW(), updated_by = p_actor_id
  WHERE id = p_review_id AND tenant_id = v_tenant_id;
END;
$fn$;
...
CREATE OR REPLACE FUNCTION fn_tpa_review_set_status(
  p_actor_id BIGINT, p_review_id BIGINT, p_status TEXT, p_notes TEXT
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE v_tenant_id UUID;
BEGIN
  v_tenant_id := current_setting('app.current_tenant_id', true)::uuid;
  IF NOT fn_current_user_has_permission('tpa.review.amend') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;
  IF p_status NOT IN ('awaiting_review','reviewed','redline_sent','closed_accepted','closed_rejected') THEN
    RAISE EXCEPTION 'invalid status %', p_status USING ERRCODE = '22023';
  END IF;
  UPDATE tpa_review SET status = p_status,
...
CREATE OR REPLACE FUNCTION fn_tpa_finding_update_verdict(
  p_actor_id BIGINT, p_finding_id BIGINT, p_user_verdict TEXT,
  p_user_redline TEXT, p_user_notes TEXT, p_resolution TEXT
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE v_tenant_id UUID; v_review_id BIGINT;
BEGIN
  v_tenant_id := current_setting('app.current_tenant_id', true)::uuid;
  IF NOT fn_current_user_has_permission('tpa.review.amend') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  ...
  UPDATE tpa_review_finding SET
    ...
    updated_at = NOW(), updated_by = p_actor_id
  WHERE id = p_finding_id AND tenant_id = v_tenant_id AND is_active = TRUE
...
CREATE OR REPLACE FUNCTION fn_tpa_review_list(
  p_actor_id BIGINT, p_status TEXT, p_limit INTEGER, p_offset INTEGER
) RETURNS JSONB LANGUAGE plpgsql STABLE
AS $fn$
DECLARE
  v_data JSONB; v_total INTEGER;
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      'id', r.id, 'referenceCode', r.reference_code,
      'counterpartyName', r.counterparty_name, 'agreementTitle', r.agreement_title,
      'agreementType', r.agreement_type, 'status', r.status,
      'overallVerdict', r.overall_verdict, 'overallRisk', r.overall_risk,
      'riskScore', r.risk_score, 'acceptCount', r.accept_count,
      'amendCount', r.amend_count, 'rejectCount', r.reject_count,
      'conflictCount', r.conflict_count, 'createdAt', r.created_at,
      ...
      'playbookNameEn', (SELECT name_en FROM tpa_playbook WHERE id = r.playbook_id)
    ) AS item
    FROM tpa_review r
    WHERE r.tenant_id = current_setting('app.current_tenant_id', true)::uuid
      AND r.is_active = TRUE
      AND (p_status IS NULL OR p_status = '' OR r.status = p_status)
    ORDER BY r.created_at DESC LIMIT v_lim OFFSET v_off
  ) sub;
  RETURN jsonb_build_object('data', v_data,
    'pagination', jsonb_build_object('total', v_total, 'limit', v_lim,
      'offset', v_off, 'hasMore', v_off + v_lim < v_total));
END;
$fn$;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'agreementTitle', r.agreement_title, 'agreementType', r.agreement_type,
    'status', r.status,
    'overallVerdict', r.overall_verdict, 'overallRisk', r.overall_risk,
    'riskScore', r.risk_score, 'acceptCount', r.accept_count,
    'amendCount', r.amend_count, 'rejectCount', r.reject_count,
    'conflictCount', r.conflict_count, 'executiveSummary', r.executive_summary,
    'llmModelVersion', r.llm_model_version, 'llmAnalysedAt', r.llm_analysed_at,
    'llmError', r.llm_error, 'notes', r.notes,
    'createdAt', r.created_at, 'updatedAt', r.updated_at,
    'createdBy', r.created_by,
    'createdByName', (SELECT concat_ws(' ', u.first_name, u.last_name)
                     FROM "user" u WHERE u.id = r.created_by),
    'playbook', (SELECT jsonb_build_object(
                   'id', p.id, 'playbookKey', p.playbook_key,
                   'agreementType', p.agreement_type, 'nameEn', p.name_en,
                   'nameAr', p.name_ar, 'version', p.version)
                 FROM tpa_playbook p WHERE p.id = r.playbook_id),
    'findings', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', f.id, 'playbookClauseId', f.playbook_clause_id,
        'clauseKey', f.clause_key, 'clauseTitle', f.clause_title,
        'displayOrder', f.display_order,
        'extractedText', f.extracted_text, 'extractedLocation', f.extracted_location,
        'aiVerdict', f.ai_verdict, 'aiRationale', f.ai_rationale,
        'aiSeverity', f.ai_severity, 'aiSuggestedRedline', f.ai_suggested_redline,
        'aiConflictsWith', f.ai_conflicts_with,
        'userVerdict', f.user_verdict, 'userRedline', f.user_redline,
        'userNotes', f.user_notes, 'resolutionStatus', f.resolution_status,
        'playbookStandard', (SELECT pc.standard_position FROM tpa_playbook_clause pc
                             WHERE pc.id = f.playbook_clause_id),
        'playbookFallback', (SELECT pc.fallback_position FROM tpa_playbook_clause pc
                             WHERE pc.id = f.playbook_clause_id),
        'playbookCriticality', (SELECT pc.criticality FROM tpa_playbook_clause pc
                                WHERE pc.id = f.playbook_clause_id)
    ...
    'documents', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', d.id, 'documentKind', d.document_kind,
        'fileName', d.file_name, 'mimeType', d.mime_type,
        'sizeBytes', d.size_bytes, 'storageUri', d.storage_uri,
        'pageCount', d.page_count, 'extractionEngine', d.extraction_engine,
        'createdAt', d.created_at
      ) ORDER BY d.created_at DESC)
      FROM tpa_review_document d WHERE d.review_id = r.id AND d.is_active = TRUE
    ), '[]'::jsonb)
  ) INTO v_result
  FROM tpa_review r
  WHERE r.id = p_id AND r.tenant_id = current_setting('app.current_tenant_id', true)::uuid
    AND r.is_active = TRUE;
  IF v_result IS NULL THEN
...
VALUES (519, 'tpa_fix_permission_fn', CURRENT_TIMESTAMP)
