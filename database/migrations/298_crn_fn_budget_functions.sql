-- Migration: 298_crn_fn_budget_functions.sql
-- Module: CR-N — Services-Contract Budget Burn (M21 Financial Intelligence, Cost half)
-- Description: ALL 9 CR-N fn_ functions — analytics reads (burn_compute, variance_for_contract,
--              year_end_projection, burn_portfolio), entity reads (budget_list, budget_get,
--              cost_actual_list, cost_actual_get_by_id), entity write (cost_actual_record).
--              Security: STABLE INVOKER for all reads; VOLATILE INVOKER for write.
--              S2-21: COMMENT + REVOKE PUBLIC + GRANT neondb_owner on every fn.
--              S2-24: CTE-split — no nested aggregates.
--              S2-25: every RAISE has USING ERRCODE.
--              S2-26: WHEN OTHERS preserves SQLSTATE.
...
-- ============================================================
-- 1. fn_contract_cost_actual_get_by_id — STABLE INVOKER internal helper
--    Used by fn_contract_cost_actual_record to return the upserted row.
-- ============================================================

CREATE OR REPLACE FUNCTION fn_contract_cost_actual_get_by_id(
  p_actor_id  BIGINT,
  p_id        BIGINT
)
RETURNS JSONB
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'createdBy',      ca.created_by,
    'updatedBy',      ca.updated_by,
    'isActive',       ca.is_active
  )
  INTO v_result
  FROM contract_cost_actual ca
  WHERE ca.id = p_id AND ca.is_active = TRUE;

  RETURN v_result;
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'fn_contract_cost_actual_get_by_id: %', SQLERRM
      USING ERRCODE = SQLSTATE;
END;
$$;

COMMENT ON FUNCTION fn_contract_cost_actual_get_by_id(BIGINT, BIGINT) IS
  'CR-N internal helper: returns one contract_cost_actual row as JSONB for use by fn_contract_cost_actual_record return path.';
...
-- ============================================================
-- 2. fn_contract_budget_list — STABLE INVOKER
--    Paginated budget lines (optionally scoped to contract / FY / category).
-- ============================================================
...
COMMENT ON FUNCTION fn_contract_budget_list(BIGINT, BIGINT, INTEGER, TEXT, INTEGER, INTEGER) IS
  'CR-N M21: Paginated budget lines optionally scoped by contract/fiscal_year/cost_category. STABLE INVOKER — RLS enforces finance.budget.read permission.';
...
-- ============================================================
-- 3. fn_contract_budget_get — STABLE INVOKER
--    One budget line by id with embedded contract summary.
-- ============================================================
...
COMMENT ON FUNCTION fn_contract_budget_get(BIGINT, BIGINT) IS
  'CR-N M21: One budget line by id with embedded contract summary. Returns NULL if not found (controller → 404). STABLE INVOKER.';
...
-- ============================================================
-- 4. fn_contract_cost_actual_list — STABLE INVOKER
--    Paginated actual-spend lines with optional filters.
-- ============================================================
...
COMMENT ON FUNCTION fn_contract_cost_actual_list(BIGINT, BIGINT, INTEGER, TEXT, VARCHAR, INTEGER, INTEGER) IS
  'CR-N M21: Paginated actual-spend lines with optional filters by contract/FY/category/period. STABLE INVOKER.';
...
-- ============================================================
-- 5. fn_contract_cost_actual_record — VOLATILE INVOKER (WRITE)
--    Record a single actual-spend line with upsert semantics.
--    Permission: finance.budget.manage (defence-in-depth on top of RLS).
-- ============================================================

CREATE OR REPLACE FUNCTION fn_contract_cost_actual_record(
  p_actor_id    BIGINT,
...
DECLARE
  ...
  v_result        JSONB;
BEGIN
  -- 1. Permission check (defence-in-depth; RLS WITH CHECK also enforces)
  IF NOT fn_current_user_has_permission('finance.budget.manage') THEN
    RAISE EXCEPTION 'fn_contract_cost_actual_record: insufficient privilege — finance.budget.manage required'
      USING ERRCODE = '42501';
  END IF;
  ...
  v_period_label := p_data->>'periodLabel';
  IF v_period_label IS NULL OR v_period_label = '' THEN
    RAISE EXCEPTION 'periodLabel is required'
      USING ERRCODE = '22023';
  END IF;

  IF (p_data->>'fiscalYear') IS NULL THEN
    RAISE EXCEPTION 'fiscalYear is required'
      USING ERRCODE = '22023';
  END IF;
  v_fiscal_year := (p_data->>'fiscalYear')::INTEGER;

  v_cost_category := p_data->>'costCategory';
  IF v_cost_category IS NULL OR v_cost_category = '' THEN
    RAISE EXCEPTION 'costCategory is required'
      USING ERRCODE = '22023';
  END IF;

  IF v_cost_category NOT IN ('day_rate','manpower','equipment','milestone','other') THEN
    RAISE EXCEPTION 'costCategory must be one of: day_rate, manpower, equipment, milestone, other'
      USING ERRCODE = '23514';
  END IF;

  IF (p_data->>'actualAmountAed') IS NULL THEN
    RAISE EXCEPTION 'actualAmountAed is required'
      USING ERRCODE = '22023';
  END IF;
  v_amount := (p_data->>'actualAmountAed')::NUMERIC(18,2);
  IF v_amount < 0 THEN
    RAISE EXCEPTION 'actualAmountAed must be >= 0'
      USING ERRCODE = '22023';
  END IF;

  -- 3. Optional fields with defaults
  v_source := COALESCE(p_data->>'source', 'manual');
  IF v_source NOT IN ('erp_feed','manual') THEN
    RAISE EXCEPTION 'source must be one of: erp_feed, manual'
      USING ERRCODE = '23514';
  END IF;

  v_period_type := COALESCE(p_data->>'periodType', 'month');
  IF v_period_type NOT IN ('month','quarter','year') THEN
    RAISE EXCEPTION 'periodType must be one of: month, quarter, year'
      USING ERRCODE = '23514';
  END IF;

  -- reference_no: coalesce NULL → '' (idempotency key requires NOT NULL)
  v_reference_no := COALESCE(p_data->>'referenceNo', '');
  ...
  IF NOT v_contract_exists THEN
    RAISE EXCEPTION 'contract not found or inactive (id=%)', p_contract_id
      USING ERRCODE = 'P0002';
  END IF;
  ...
  -- 6. Upsert (idempotent re-post of same ERP voucher)
  INSERT INTO contract_cost_actual (
    ...
    recorded_at, notes, data_classification, created_by, updated_by, is_active
  ) VALUES (
    v_tenant_id, p_contract_id, v_period_type, v_period_label, v_fiscal_year,
    v_cost_category, v_amount, 'AED', v_source, v_reference_no,
    NOW(), v_notes, 'demo', v_actor, v_actor, TRUE
  )
  ON CONFLICT ON CONSTRAINT contract_cost_actual_idempotency_key
  DO UPDATE SET
  ...
  SELECT fn_contract_cost_actual_get_by_id(p_actor_id, v_id) INTO v_result;
  RETURN v_result;

EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'fn_contract_cost_actual_record: %', SQLERRM
      USING ERRCODE = SQLSTATE;
END;
$$;

COMMENT ON FUNCTION fn_contract_cost_actual_record(BIGINT, BIGINT, JSONB) IS
  'CR-N M21: Record a single actual-spend line (manual entry; mirrors ERP-feed shape). VOLATILE INVOKER + explicit finance.budget.manage check (defence-in-depth). Upsert semantics via idempotency key.';
...
-- ============================================================
-- 6. fn_budget_burn_compute — STABLE INVOKER
--    Per-period × category budget-vs-actual, variance, cumulative burn.
--    S2-24: budget_q / actual_m / actual_roll / joined CTEs (split-aggregate).
-- ============================================================
...
BEGIN
  ...
  WITH budget_q AS (
    -- SUM allocated by (period_label, cost_category) — quarter grain
  ...
  actual_m AS (
    -- SUM actual by (period_label, cost_category) — month grain
    SELECT period_label, cost_category,
  ...
  actual_roll AS (
    -- Map month period_label → owning quarter, then aggregate to quarter+category
    SELECT
      CASE
  ...
  periods AS (
    -- All distinct quarter period_labels
  ...
  period_cat_budget AS (
    -- period × category budget (pre-aggregated)
  ...
  period_cat_actual AS (
    -- period × category actual rolled up to quarter
  ...
  category_rows AS (
    -- per-category rows with variance for jsonb_agg (single-level)
    SELECT pcb.period_label, pcb.fiscal_year, pcb.cost_category,
  ...
  period_totals AS (
    -- period-level totals (single aggregation over category_rows)
  ...
  -- Cumulative burn (quarter grain, running sum ordered by period_label)
  WITH period_agg AS (
    SELECT period_label,
...
COMMENT ON FUNCTION fn_budget_burn_compute(BIGINT, BIGINT) IS
  'CR-N M21: Per-period × category budget-vs-actual, variance, cumulative burn for one contract. STABLE INVOKER — RLS gates via finance.budget.read. S2-24: split-aggregate CTEs. Returns NULL if contract absent.';
...
-- ============================================================
-- 7. fn_budget_variance_for_contract — STABLE INVOKER
--    Breaches above threshold + correlated cure_period/LD clause refs.
-- ============================================================
...
BEGIN
  ...
  -- Variance breaches: compare month actuals rolled up to quarter vs quarterly budget
  -- S2-24: budget_q / actual_roll CTEs (single aggregation each)
  WITH budget_q AS (
  ...
  joined AS (
    SELECT bq.period_label, bq.cost_category, bq.fiscal_year,
           bq.budget_aed,
           COALESCE(ar.actual_aed, 0) AS actual_aed,
           COALESCE(ar.actual_aed, 0) - bq.budget_aed AS variance_aed,
           CASE WHEN bq.budget_aed > 0
                THEN ROUND(((COALESCE(ar.actual_aed, 0) - bq.budget_aed) / bq.budget_aed * 100)::NUMERIC, 2)
                ELSE NULL END AS variance_pct
    FROM budget_q bq
    LEFT JOIN actual_roll ar ON ar.quarter_label = bq.period_label AND ar.cost_category = bq.cost_category
    WHERE bq.budget_aed > 0
      AND CASE WHEN bq.budget_aed > 0
               THEN ((COALESCE(ar.actual_aed, 0) - bq.budget_aed) / bq.budget_aed * 100) >= (v_threshold_pct / 2)
               ELSE FALSE END
  )
  SELECT
  ...
  -- Clause refs (non-aggregated subquery — S2-24 no nesting)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'clauseId',       cce.id,
...
COMMENT ON FUNCTION fn_budget_variance_for_contract(BIGINT, BIGINT, NUMERIC) IS
  'CR-N M21: List (period, category) breaching the variance threshold + correlated cure_period + liquidated_damages clause refs. STABLE INVOKER. Returns NULL if contract absent. S2-24: split-aggregate CTEs.';
...
-- ============================================================
-- 8. fn_budget_year_end_projection — STABLE INVOKER
--    Run-rate extrapolation → projected FY-end spend vs allocated.
-- ============================================================
...
COMMENT ON FUNCTION fn_budget_year_end_projection(BIGINT, BIGINT, VARCHAR) IS
  'CR-N M21: Run-rate extrapolation → projected FY-end spend vs allocated budget. STABLE INVOKER. Returns NULL if contract absent; returns insufficient_data when monthsElapsed=0 (no divide-by-zero).';
...
-- ============================================================
-- 9. fn_budget_burn_portfolio — STABLE INVOKER
--    Portfolio rollup across all budgeted contracts.
--    S2-24 CRITICAL: budget_by_contract / actual_by_contract / joined (split-aggregate).
-- ============================================================
...
COMMENT ON FUNCTION fn_budget_burn_portfolio(BIGINT, JSONB) IS
  'CR-N M21: Portfolio rollup across all budgeted contracts (finance + executive). S2-24: split-aggregate CTEs. Never returns NULL. STABLE INVOKER.';
...
VALUES (298, '298_crn_fn_budget_functions', CURRENT_TIMESTAMP)
...
-- ============================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS fn_budget_burn_portfolio(BIGINT, JSONB);
-- DROP FUNCTION IF EXISTS fn_budget_year_end_projection(BIGINT, BIGINT, VARCHAR);
-- DROP FUNCTION IF EXISTS fn_budget_variance_for_contract(BIGINT, BIGINT, NUMERIC);
-- DROP FUNCTION IF EXISTS fn_budget_burn_compute(BIGINT, BIGINT);
-- DROP FUNCTION IF EXISTS fn_contract_cost_actual_record(BIGINT, BIGINT, JSONB);
-- DROP FUNCTION IF EXISTS fn_contract_cost_actual_list(BIGINT, BIGINT, INTEGER, TEXT, VARCHAR, INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS fn_contract_budget_get(BIGINT, BIGINT);
-- DROP FUNCTION IF EXISTS fn_contract_budget_list(BIGINT, BIGINT, INTEGER, TEXT, INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS fn_contract_cost_actual_get_by_id(BIGINT, BIGINT);
-- DELETE FROM schema_migrations WHERE version = 298;
-- COMMIT;
-- ============================================================
