-- Migration: 395_layla_fix_advisory_draft_get_correlation_rule_join.sql
-- Unit: Layla Counsel QA Phase 3.7 — fix type mismatch on rule join
-- correlation.rule_id is text; correlation_rule.rule_id is text — join on rule_id, not id.
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'sourceCorrelation', (
      ...
      FROM correlation co
      LEFT JOIN correlation_rule cr ON cr.rule_id = co.rule_id   -- L26 FIX: join on rule_id (text), not id (bigint)
