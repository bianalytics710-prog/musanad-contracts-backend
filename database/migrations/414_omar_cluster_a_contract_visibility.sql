-- Migration: 414_omar_cluster_a_contract_visibility.sql
-- Unit: Omar Operations QA Phase 3 — Cluster B (contract scope + HERO-001 visibility)
-- Targets:
--   O11  Omar's contracts list shows 21 rows all drafted_by=Omar. Operations
--        role isn't a contract-drafter role. Re-distribute 16 of Omar's 21
--        drafted contracts back to Dana Drafter / Aisha Approver / Layla
--        Counsel; keep ~5 for him.
--   O12  HERO-001 (id 52) is invisible in Omar's list — operations role isn't
--        in v_role_can_see_all. Add 'operations' to the role gate so Omar's
--        contracts browse becomes wide (Scene F.3 anchor: "Same browse as
--        executive."). HERO-001 will surface as part of the wide read.
--   O13  No expiring-soon contracts in Omar's portfolio — side-effect fix:
--        with the wide read, the existing expiring-soon rollup will populate
--        from the org-wide register.
--
-- Body of fn_contract_list otherwise byte-for-byte identical to migration
-- 087. Same 21-arg signature, same projection, same indexes. Only the
-- v_role_can_see_all list changes (5 → 6 roles).
...
DECLARE
  ...
  v_aisha_id   BIGINT := 6;
  v_layla_id   BIGINT := 4;
  v_fatima_id  BIGINT := 13;
  v_khalid_id  BIGINT := 14;
  v_pari_id    BIGINT := 15;
BEGIN
  ----------------------------------------------------------------------------
  -- O11 — Redistribute Omar's draft load away from him.
  -- Per QA audit: an Operations role drafting 21 of 21 visible contracts
  -- is implausible. Keep 5 contracts where Omar makes operational sense
  -- (services + EPC), move the rest to Dana / Aisha / Layla / Fatima /
  -- Khalid / Pari so the drafter column reads diverse for everyone.
  --
  -- Guard: skip in environments where seed personas don't exist (test branch).
  ----------------------------------------------------------------------------
  IF EXISTS (SELECT 1 FROM "user" WHERE id IN (4, 5, 6, 12, 13, 14, 15)) THEN
    -- Move ~16 of Omar's drafts to other personas via a deterministic ROW_NUMBER
    -- assignment. Order by id so the redistribution is stable across re-applies.
    WITH omar_drafts AS (
      SELECT id, ROW_NUMBER() OVER (ORDER BY id) AS rn
        FROM contract
        WHERE drafted_by = v_omar_id AND is_active = TRUE
    ),
    reassignment AS (
      SELECT
        id,
        CASE (rn - 1) % 6
          WHEN 0 THEN v_dana_id    -- Dana Drafter (canonical drafter)
          WHEN 1 THEN v_aisha_id   -- Aisha Approver
          WHEN 2 THEN v_layla_id   -- Layla Counsel
          WHEN 3 THEN v_fatima_id  -- Fatima Finance
          WHEN 4 THEN v_khalid_id  -- Khalid Compliance
          WHEN 5 THEN v_pari_id    -- Pari Procurement
        END AS new_drafter
      FROM omar_drafts
      WHERE rn <= 16  -- redistribute 16; keep ~5 with Omar
    )
    UPDATE contract c
       SET drafted_by = r.new_drafter,
           updated_at = NOW(),
           updated_by = 1
      FROM reassignment r
     WHERE c.id = r.id
       AND c.drafted_by = v_omar_id;

    RAISE NOTICE 'mig 414 O11: redistributed up to 16 contracts away from Omar (id=12).';
  ELSE
    RAISE NOTICE 'mig 414 O11: persona users missing, skipping redistribute.';
  END IF;

  ----------------------------------------------------------------------------
  -- O12/O13 — Drop and recreate fn_contract_list with 'operations' added to
  -- v_role_can_see_all. Body byte-for-byte mirrors 087 except for line 91.
  ----------------------------------------------------------------------------
  FOR rec IN
...
BEGIN
  ...
  -- O12: add 'operations' to the role gate (Scene F.3 anchor: same browse as executive).
...
VALUES (414, '414_omar_cluster_a_contract_visibility — O11/O12/O13 operations role visibility + drafter redistribute', NOW())
ON CONFLICT (version) DO NOTHING;

-- ROLLBACK: re-apply migration 087.
