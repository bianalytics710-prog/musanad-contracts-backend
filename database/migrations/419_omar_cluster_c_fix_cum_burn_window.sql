-- Migration: 419_omar_cluster_c_fix_cum_burn_window.sql
-- Unit: Omar Operations QA Phase 3 — Cluster C followup #2
-- Targets: re-apply DEFECT-298-1 fix on top of mig 417 — cumulative burn
-- block in fn_budget_burn_compute had window-fn-inside-aggregate which mig
-- 417 inadvertently reverted from the mig 306 fix. Re-introduce the cum_burn
-- CTE wrap while preserving the mig 417 FY-scoping.
...
BEGIN
  ...
  -- Re-apply DEFECT-298-1 fix: split window into cum_burn CTE before jsonb_agg.
  WITH period_agg AS (
  ...
  RETURN jsonb_build_object(
    ...
    'currency',           'AED',
    'fiscalYear',         v_fy,
    -- mig 306 renamed keys: totalBudgetedAed (with -ed), burnRatePct, +byQuarter alias
...
VALUES (419, '419_omar_cluster_c_fix_cum_burn_window — re-apply DEFECT-298-1 window fix on FY-scoped fn', NOW())
