-- 513_advisory_clause_title_default_fix.sql
-- ============================================================================
-- Cosmetic fix: mig 512 default clause_title was "the relevant", which
-- combined with the template's leading "the" produces "the the relevant
-- clause". Change the default to "applicable" (reads as "the applicable
-- clause") and re-render all unapproved drafts.
-- ============================================================================
BEGIN;
...
BEGIN
  ...
  FOR v_correlation IN
    SELECT
      ...
      CASE
        WHEN cce.clause_type_v2 IS NULL THEN 'applicable'
...
-- Re-regenerate existing unapproved drafts with the fixed default.
...
BEGIN
  ...
  LOOP
    v_ctx := jsonb_build_object(
      ...
      'clause_title',         CASE
                                WHEN v_d.clause_type_v2 IS NULL THEN 'applicable'
  ...
  RAISE NOTICE 're-regenerated % unapproved advisory drafts', v_count;
END
$regen$;


INSERT INTO schema_migrations (version, description, applied_at)
VALUES (513, 'advisory_clause_title_default_fix', CURRENT_TIMESTAMP)
