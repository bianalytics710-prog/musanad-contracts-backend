-- Migration: 165_fix_fn_obligations_derive_recurrence.sql
-- Module: M12 / CR-D — third defect-fix on fn_obligations_derive_from_clause
--
-- DEFECTS chain (surfaced sequentially during post-delivery browser walk):
--   163 — c.effective_date → c.start_date (column never existed on contract)
--   164 — title_en added to all 5 INSERTs (NOT NULL column was being skipped)
--   165 — recurrence values must match the CHECK constraint
--           (once / monthly / quarterly / annually). The fn was passing
--           'none' (4 branches) and 'annual' (ICV branch); both fail the
--           CHECK. Replace with 'once' and 'annually'.
--
-- This migration consolidates 163 + 164 + 165 fixes — the fn body below is
-- the single authoritative version after all three rounds.
--
  ...
  ELSIF v_clause_type = 'term_and_renewal' THEN
    IF (v_params ? 'expiry_date') AND (v_params ? 'renewal_notice_period_days') THEN
      ...
      INSERT INTO contract_obligation (
        ...
        v_due_date, 'principal', 'once', 'active',
        p_clause_id,
        NOW(), NOW(), p_actor_id, p_actor_id, TRUE
      )
      ON CONFLICT (derived_from_clause_id, obligation_type)
  ...
  ELSIF v_clause_type = 'icv_in_country_value' THEN
    IF (v_params ? 'icv_reporting_period_months') AND (v_contract.start_date IS NOT NULL) THEN
      ...
      INSERT INTO contract_obligation (
        ...
        v_due_date, 'contractor', 'annually', 'active',
        p_clause_id,
        NOW(), NOW(), p_actor_id, p_actor_id, TRUE
      )
      ON CONFLICT (derived_from_clause_id, obligation_type)
...
INSERT INTO schema_migrations(version, description) VALUES (164, '165_fix_fn_obligations_derive_recurrence') ON CONFLICT DO NOTHING;
