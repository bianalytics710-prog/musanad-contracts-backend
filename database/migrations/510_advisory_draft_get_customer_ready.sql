-- 510_advisory_draft_get_customer_ready.sql
-- ============================================================================
-- Purpose: Make fn_advisory_draft_get_by_id return enough data for a
--   customer-facing detail view:
--     1. matchedClauses now includes a proper verbatim snippet pulled from
--        contract_clause_extracted.text_excerpts (was using summary_en —
--        identical to clauseTitle and unhelpful).
--     2. clauseTitle is humanised ("Force Majeure" instead of "force_majeure").
--     3. riskScoreSummary now embeds the per-dimension breakdown + weights so
--        the FE can show "how this score was calculated" without a second
--        round-trip to fn_risk_score_explain.
--
-- Pure read-fn replacement — no schema changes.
-- ============================================================================
BEGIN;
...
BEGIN
  SELECT jsonb_build_object(
    ...
    -- riskScoreSummary now exposes dimension breakdown + weights for the FE
    -- "how is this score calculated?" tooltip. Falls back gracefully when the
    -- explanation JSONB shape is older or missing.
    'riskScoreSummary', (
      SELECT jsonb_build_object(
    ...
    -- matchedClauses — clauseTitle humanised from clause_type_v2; snippet pulled
    -- from text_excerpts (verbatim source text), falling back to summary_en
    -- when text_excerpts is empty. This is what the customer will actually
    -- read in the advisory preview.
    'matchedClauses', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        END,
        'snippet', COALESCE(
          -- text_excerpts is a JSONB like {"param1": "verbatim text...", "param2": "..."}.
          -- Concatenate the first 2 non-null values into one snippet, capped at
          -- ~280 chars so it fits in the FE blockquote.
          (
...
VALUES (510, 'advisory_draft_get_customer_ready', CURRENT_TIMESTAMP)
...
-- ROLLBACK BEGIN
-- ============================================================================
-- Re-apply migration 395 to restore the previous fn body. We intentionally do
-- not paste the prior body verbatim here — the rollback path is "re-run 395".
-- ROLLBACK END
