-- Migration: 361_eman_enrich_event_feed_and_expand_activity_types.sql
-- Unit: Eman Executive QA Phase 3.3 (2026-05-31)
-- Fix:
--   E12 — Executive events feed shows 8 identical "AI risk score updated"
--         entries. Root cause: fn_dashboard_executive events14d block has
--         a fixed CASE-WHEN that maps every activity_type to a generic
--         label ("AI risk score updated") and ignores ca.description_en.
--         Worse, ai_risk_score_updated rows dominate the recent 14d
--         window with 100+ rows.
--
--         Two changes to the events14d block (rest of fn preserved
--         byte-for-byte from migration 321):
--           (1) Expand the activity_type WHERE clause to include the new
--               business-narrative types seeded in mig 356:
--               approval_decided, impact_signal_notify, amendment_initiated,
--               ai_summary_generated.
--           (2) Prefer ca.description_en when non-empty — falls back to
--               the existing CASE mapping when null. This lets the seeded
--               varied descriptions show through.
--           (3) Down-rank ai_risk_score_updated events in ORDER BY so
--               business events surface to the top 8.
...
BEGIN
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        -- E12 fix: prefer the activity's own description_en when populated
        -- (seeded business-narrative text) — fall back to a generic
        -- per-activity_type label when description is empty.
        COALESCE(
          NULLIF(TRIM(ca.description_en), ''),
          CASE ca.activity_type
            ...
            WHEN 'ai_summary_generated'        THEN 'AI summary generated'
            ...
            WHEN 'approval_decided'            THEN 'Approval decision recorded'
            WHEN 'impact_signal_notify'        THEN 'Impact signal notified'
            WHEN 'amendment_initiated'         THEN 'Amendment initiated'
        ...
        CASE
          WHEN ca.activity_type IN ('regulatory_impact_detected','impact_signal_notify') THEN 'high'
          WHEN ca.activity_type IN ('amendment_initiated','approval_decided','ai_summary_generated') THEN 'medium'
          WHEN ca.activity_type = 'fully_executed' THEN 'low'
          WHEN ca.activity_type = 'ai_risk_score_updated' THEN 'low'
          ELSE 'low'
        END AS severity
      FROM contract_activity ca
      JOIN contract c ON c.id = ca.contract_id AND c.is_active = TRUE
      -- E12 fix: expand activity_type whitelist to include the new
      -- business-narrative event types seeded in mig 356.
      WHERE ca.activity_type IN (
          'fully_executed','sent_for_signature','submitted_for_approval',
          'ai_risk_score_updated','ai_summary_generated',
          'regulatory_impact_detected','regulatory_impact_resolved',
          'approval_decided','impact_signal_notify','amendment_initiated')
        AND ca.created_at >= CURRENT_DATE - INTERVAL '14 days'
    ) all_events
    WHERE occurred_at IS NOT NULL
    -- E12 fix: down-rank ai_risk_score_updated so business events show
    -- first; within the same priority group, most recent first.
    ORDER BY
      CASE WHEN event_type = 'ai_risk_score_updated' THEN 1 ELSE 0 END ASC,
      occurred_at DESC
    LIMIT 8
  ) ev;
  ...
  -- NF-1 FIX (321): recentMarginChange now COMPUTED via S2-24 split-aggregate CTEs.
  --   Step 1 (pc_ranked): for every price_change snapshot within the tenant, rank by
  --          (trade_position_id, computed_at) and carry the prior row's margin via LAG().
  --   Step 2 (latest_pc_per_pos): pick only the most-recent price_change row per position.
  --   Step 3 (recent_window): keep only positions whose most-recent price_change snapshot
  --          is within 30 days.  Computes per-position delta = new_aed - prior_aed.
  --   Step 4 (delta_totals): sum deltas across all in-window positions; grab the benchmark
  --          code + computed_at from the very latest price_change event (most recent asOf).
  --          deltaUsd summed from total_margin_usd delta (FX frozen at compute time per schema).
  --   If NO recent price_change snapshot exists → NULL (graceful; FE shows nothing).
  --   No hardcoded numeric literal — Rule 8 compliant.
  -- All other aspects (margin_filtered / by_side / totals / top3) PRESERVED from mig 316.
  ...
  BEGIN
    ...
    -- NF-1 FIX: split-aggregate CTEs to compute recentMarginChange from real data.
    -- S2-24: no nested aggregate — each CTE is a single-scalar SELECT over a named set.
    pc_ranked AS (
      -- Step 1: all price_change snapshots ranked per position; LAG gives prior row's values.
      SELECT
        ms.trade_position_id,
        ms.benchmark_code_used,
        ms.total_margin_aed                                               AS new_aed,
        ms.total_margin_usd                                               AS new_usd,
        ms.computed_at,
        LAG(ms.total_margin_aed) OVER (
          PARTITION BY ms.trade_position_id
          ORDER BY ms.computed_at ASC
        )                                                                  AS prior_aed,
        LAG(ms.total_margin_usd) OVER (
          PARTITION BY ms.trade_position_id
          ORDER BY ms.computed_at ASC
        )                                                                  AS prior_usd
      FROM margin_snapshot ms
      WHERE ms.tenant_id = current_setting('app.current_tenant_id', true)::uuid
        AND ms.triggered_by = 'price_change'
    ),
    latest_pc_per_pos AS (
      -- Step 2: most-recent price_change snapshot per position (DISTINCT ON equivalent via rank).
      SELECT DISTINCT ON (trade_position_id)
        trade_position_id,
        benchmark_code_used,
        new_aed,
        new_usd,
        ...
        computed_at
      FROM pc_ranked
      ORDER BY trade_position_id, computed_at DESC
    ),
    recent_window AS (
      -- Step 3: only positions where the latest price_change is within 30 days.
      -- Per-position delta = new_aed - prior_aed (prior_aed NULL when only 1 snapshot → delta=0).
      SELECT
        trade_position_id,
        benchmark_code_used,
        computed_at,
        COALESCE(new_aed,   0) - COALESCE(prior_aed, new_aed,   0) AS delta_aed,
        COALESCE(new_usd,   0) - COALESCE(prior_usd, new_usd,   0) AS delta_usd
      FROM latest_pc_per_pos
      WHERE computed_at >= NOW() - INTERVAL '30 days'
    ),
    delta_totals AS (
      -- Step 4: aggregate deltas; pick benchmark_code + asOf from the most recent event.
      SELECT
        COALESCE(SUM(delta_aed), 0)                               AS total_delta_aed,
        COALESCE(SUM(delta_usd), 0)                               AS total_delta_usd,
        COUNT(*)                                                   AS positions_in_window,
        (SELECT rw2.benchmark_code_used
         FROM recent_window rw2
         ORDER BY rw2.computed_at DESC
         LIMIT 1)                                                  AS latest_benchmark,
        (SELECT rw2.computed_at
         FROM recent_window rw2
         ORDER BY rw2.computed_at DESC
         LIMIT 1)                                                  AS latest_computed_at
      FROM recent_window
    ),
    top3 AS (
    ...
    SELECT jsonb_build_object(
      ...
      'recentMarginChange', (
        -- NF-1 FIX: derived from delta_totals; NULL when no recent price_change snapshots exist.
        SELECT CASE WHEN dt.positions_in_window > 0 AND dt.latest_benchmark IS NOT NULL THEN
          jsonb_build_object(
            'benchmarkCode', dt.latest_benchmark,
            'deltaAed',      ROUND(dt.total_delta_aed::NUMERIC, 2)::text,
            'deltaUsd',      ROUND(dt.total_delta_usd::NUMERIC, 2)::text,
            'asOf',          dt.latest_computed_at::date
          )
        ELSE NULL END
        FROM delta_totals dt
      ),
  ...
  RETURN jsonb_build_object(
    ...
    -- CR-O: 11th additive key (tradeMarginSummary) — NF-1 FIX: recentMarginChange now computed
...
COMMENT ON FUNCTION public.fn_dashboard_executive(integer) IS
  'E12 fix (361): events14d block now prefers ca.description_en (business-narrative text seeded in mig 356) over generic labels, expands activity_type whitelist by +4 types (approval_decided/impact_signal_notify/amendment_initiated/ai_summary_generated), and down-ranks ai_risk_score_updated noise in ORDER BY so the top-8 feed shows business events first. All 11 RETURN keys preserved byte-for-byte from mig 321. NF-1 (321): tradeMarginSummary computed. CR-O (316), CR-N (299), CR-G (180): prior keys.';
...
-- ============================================================
-- Restore fn_dashboard_executive to mig 316 body (hardcoded literals).
-- BEGIN;
-- -- [Paste migration 316 CREATE OR REPLACE here]
-- DELETE FROM schema_migrations WHERE version = 321;
-- COMMIT;
-- ============================================================
