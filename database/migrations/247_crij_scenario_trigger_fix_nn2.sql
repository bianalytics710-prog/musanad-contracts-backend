-- Migration: 247_crij_scenario_trigger_fix_nn2.sql
-- Module: M17+M18 — CR-I + CR-J DEBT-CRIJ-3 fix v3
-- Description: Fix fn_demo_scenario_trigger: supply remaining NOT NULL / no-default columns
--   (source_reliability, fetched_at, confidence, raw_payload) missed in migration 246.
--   Full column list per all-NOT-NULL audit. No logic changes.
...
BEGIN
  ...
  -- Complete column list for osint_signal:
  ...
  --   source_reliability, confidence, fetched_at, raw_payload, dedup_hash,
  --   url, <optional: geographies / affected_entities>,
  --   data_classification, is_active, created_at, updated_at
  -- ================================================================
  CASE p_scenario_id

    -- ----------------------------------------------------------------
    -- hormuz: geopolitical signal. Correlations via cron.
    -- ----------------------------------------------------------------
    WHEN 'hormuz' THEN
    ...
    -- ----------------------------------------------------------------
    -- ofac_sanctions: sanctions signal. Correlations via cron.
    -- ----------------------------------------------------------------
    ...
    -- ----------------------------------------------------------------
    -- brent_review: commodity signal. Correlations via cron.
    -- ----------------------------------------------------------------
    ...
    -- ----------------------------------------------------------------
    -- epc_sla: internal milestone_slippage. Signal only.
    ...
    -- ----------------------------------------------------------------
    -- cyclone: FULLY WIRED — weather signal + fn_rule_evaluate_weather_fm_eligible + advisory draft.
    -- ----------------------------------------------------------------
    WHEN 'cyclone' THEN
      SELECT id, source_id INTO v_src_id, v_src_code FROM osint_source
    ...
    -- ----------------------------------------------------------------
    -- icv_shortfall: PARTIAL — signal + synthetic correlation + advisory draft.
    ...
    -- ----------------------------------------------------------------
    -- esg_subcontractor: PARTIAL — pick/insert ESG signal + synthetic correlation + advisory draft.
    -- ----------------------------------------------------------------
...
COMMENT ON FUNCTION fn_demo_scenario_trigger(BIGINT, TEXT) IS 'DEFINER: trigger demo scenario with per-scenario event-injection handlers (DEBT-CRIJ-3 v3 — all NOT NULL columns supplied). cyclone=FULLY WIRED (signal+fn_rule_evaluate_weather_fm_eligible+advisory_draft). icv_shortfall/esg_subcontractor=PARTIAL (signal+synthetic_correlation+advisory_draft). hormuz/ofac_sanctions/brent_review=SIGNAL-ONLY (cron correlates). epc_sla/renewal=SIGNAL-ONLY (GAP in raw_payload). Outcome JSONB reflects real counts. Requires demo.scenario.trigger permission.';
...
VALUES (247, '247_crij_scenario_trigger_fix_nn2', CURRENT_TIMESTAMP)
ON CONFLICT (version) DO NOTHING;

-- ============================================================
-- ROLLBACK
-- ============================================================
-- DELETE FROM schema_migrations WHERE version = 247;
-- Restore fn_demo_scenario_trigger from migration 246 if needed.
-- ============================================================
