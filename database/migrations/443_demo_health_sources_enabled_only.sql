-- Migration: 443_demo_health_sources_enabled_only.sql
-- Module: Demo Harness — Health Check
-- Description: The source-health worker only sweeps sources where
--              enabled=TRUE AND is_active=TRUE. The 24h-fresh denominator in
--              fn_pre_demo_health_check should match that scope; otherwise
--              disabled-but-active sources permanently drag the freshness
--              ratio below 50% and the panel reports DEGRADED forever.
-- Date: 2026-06-01
  ...
  WITH
    ...
    sources_health AS (
      SELECT 'sources'::TEXT AS name,
        ...
        CASE
          WHEN total = 0 THEN 'No enabled OSINT sources configured'
...
COMMENT ON FUNCTION fn_pre_demo_health_check(BIGINT) IS 'STABLE INVOKER: 9-subsystem health aggregator. Sources denominator scoped to enabled+active sources (worker scope match); OK at >=50% fresh in last 24h. Scoring OK within 7d. Notification OK unless 5+ recent failures. BE merges storage/openai/smtp HTTP probes.';
...
VALUES (443, '443_demo_health_sources_enabled_only', CURRENT_TIMESTAMP);

-- ============================================================
-- ROLLBACK
-- ============================================================
-- DELETE FROM schema_migrations WHERE version = 443;
-- (Re-apply 442_demo_health_check_pragmatic.sql to revert to all-is_active denominator.)
-- ============================================================
