-- Migration: 507_fn_contract_list_fix_orderby.sql
-- Module: Contracts — fn_contract_list pagination ordering bug
-- Date: 2026-06-03
--
-- BUG: fn_contract_list's inner SELECT applies `LIMIT p_limit OFFSET v_offset`
-- WITHOUT an ORDER BY clause. The ORDER BY only appears in the OUTER
-- jsonb_agg(... ORDER BY ord_key1, ord_key2, ord_key3) — which sorts the
-- already-limited 20-row subset, not the full result set.
--
-- Symptom (reported 2026-06-03): a freshly-composed contract (id 676,
-- updated_at = today) didn't appear in the drafter's /api/v1/contracts
-- response despite total = 169 and the contract passing the visibility
-- filter directly. Postgres returned an arbitrary 20 rows from the
-- visible set (heap-order) and then sorted those, so the newest row
-- never made the page.
--
-- FIX: push the ORDER BY into the inner SELECT so LIMIT/OFFSET operate
-- on a properly-sorted relation. The outer jsonb_agg sort is now
-- redundant but harmless.
--
-- Rollback block at bottom of file.
...
BEGIN
  ...
  -- ── DATA PAGE — ORDER BY pushed INSIDE the inner SELECT so LIMIT/OFFSET
  --    operate on a sorted relation. The outer jsonb_agg ORDER BY is now
  --    redundant but kept for defensive consistency.
...
VALUES (507, '507_fn_contract_list_fix_orderby', CURRENT_TIMESTAMP)
ON CONFLICT (version) DO NOTHING;

COMMIT;

-- ROLLBACK BEGIN
-- The previous fn body (LIMIT-without-ORDER-BY) cannot be restored from this
-- file alone — it must be recovered from migration 484 (or wherever the
-- prior CREATE OR REPLACE lives). This rollback only removes the marker:
DELETE FROM schema_migrations WHERE version = 507;
-- ROLLBACK END
