...
export const suggestClauses = async (args: {
  ...
  let validated = aiDraftingAssistantSuggestToolSchema.safeParse(parsed);
  if (!validated.success) {
    // Salvage path — the LLM sometimes returns a flat array, a shape with
    // different field casing, or a single suggestion at the root. Coerce
    // common variants into the expected envelope before giving up entirely.
    const candidates: Array<Record<string, unknown>> = (() => {
      if (Array.isArray(parsed)) return parsed as Array<Record<string, unknown>>;
      const p = parsed as Record<string, unknown>;
      if (Array.isArray(p?.suggestions)) return p.suggestions as Array<Record<string, unknown>>;
      if (Array.isArray(p?.clauses)) return p.clauses as Array<Record<string, unknown>>;
      if (Array.isArray(p?.items)) return p.items as Array<Record<string, unknown>>;
      // Single-suggestion fallback
      if (typeof p?.proposedText === 'string' || typeof p?.text === 'string') return [p];
      return [];
    })();

    const allowedKinds = new Set(['missing_clause', 'weak_clause', 'regulatory']);
    const coerced = {
      suggestions: candidates.slice(0, 4).map((c) => ({
        kind: allowedKinds.has(String(c.kind)) ? (c.kind as string) : 'regulatory',
        title: String(c.title ?? c.name ?? 'AI suggestion').slice(0, 300) || 'AI suggestion',
        rationale: String(c.rationale ?? c.why ?? c.reason ?? 'Generated by drafting assistant.').slice(
          0,
          2000,
        ) || 'Generated by drafting assistant.',
        proposedText: String(c.proposedText ?? c.text ?? c.body ?? c.content ?? '').slice(0, 8000) || ' ',
      })),
    };

    const recheck = aiDraftingAssistantSuggestToolSchema.safeParse(coerced);
    if (!recheck.success) {
